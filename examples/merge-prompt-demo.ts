...
async function createExampleFiles() {
  ...
  try {
    ...
    await fs.writeFile(
      path.join(examplesDir, 'prprc.md'),
      `# PRPRC System Instructions
...
`
    ...
    await fs.writeFile(
      path.join(examplesDir, 'agent.md'),
      `# Agent Instructions
...
`
    ...
    await fs.writeFile(
      path.join(examplesDir, 'inspector.md'),
...
`
    ...
    await fs.writeFile(
      path.join(examplesDir, 'orchestrator.md'),
...
async function demonstrateAgentPrompt() {
  ...
  const agentPrompt = await buildAgentPrompt(
    path.join(examplesDir, 'prprc.md'),
    [{ instructions_path: path.join(examplesDir, 'agent.md') }],
    {
      agentId: 'claude-code',
...
- Code quality indicators positive
  ...
  const inspectorPrompt = await buildInspectorPrompt(
    path.join(examplesDir, 'prprc.md'),
    { instructions_path: path.join(examplesDir, 'inspector.md') },
    scannerData,
    previousContext,
    {
...
- Build pipeline: healthy
  ...
  const orchestratorPrompt = await buildOrchestratorPrompt(
    path.join(examplesDir, 'prprc.md'),
    { instructions_path: path.join(examplesDir, 'orchestrator.md') },
    inspectorPayload,
    prpContext,
    sharedContext,
    {
...
async function demonstrateTOON() {
  ...
  const toonString = TOON.minify(largeObject);
  const parsedBack = TOON.parse(toonString);
...
export {
  ...
  demonstrateAgentPrompt,
  demonstrateInspectorPrompt,
  demonstrateOrchestratorPrompt,
  demonstrateTOON,
  demonstrateCaching
};