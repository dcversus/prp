...
export class ErrorHandler {
  ...
  private getExitCode(error: Error): number {
    ...
    // Generic error codes
    if (error.name === 'ValidationError') return 3;
    if (error.name === 'TypeError') return 4;
    if (error.name === 'ReferenceError') return 5;
    if (error.name === 'SyntaxError') return 6;
    if (error.name === 'RangeError') return 7;
