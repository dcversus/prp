...
export class SecurityCompliance {
  ...
  private async runAutomatedSecurityTest(control: SecurityControl): Promise<boolean> {
    ...
    try {
      ...
      console.warn(`Security test failed for control ${control.id}:`, error);
      return false;
    }
  }
  ...
  private calculatePriority(
    ...
    if (requirementLevel === 'mandatory') priority += 100;
    if (requirementLevel === 'recommended') priority += 50;

    if (implementationStatus === 'not_implemented') priority += 50;
    if (implementationStatus === 'planned') priority += 25;
    if (implementationStatus === 'partial') priority += 10;
