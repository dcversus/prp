...
export class TokenMetricsStream extends EventEmitter {
  ...
  addAlert(alert: Omit<TokenAlert, 'id' | 'timestamp'>): void {
    ...
    this.agentAlerts.get(alert.agentId)!.push(fullAlert);
  ...
  acknowledgeAlert(alertId: string): void {
    for (const [agentId, alerts] of this.agentAlerts.entries()) {
  ...
  private loadHistoricalData(): void {
    ...
    try {
      ...
      if (existsSync(dataPath)) {
        ...
        Object.entries(data).forEach(([agentId, dataPoints]: [string, any]) => {
          this.historicalData.set(agentId, dataPoints.map((dp: any) => ({
            ...dp,
  ...
  private getAgentStatuses(): AgentTokenStatus[] {
    ...
    this.subscribers.forEach((_, agentId) => {
      ...
      if (latestData) {
        ...
        if (percentage >= 95) status = 'blocked';
  ...
  private extractAgentType(agentId: string): string {
    if (agentId.includes('scanner')) return 'scanner';
    if (agentId.includes('inspector')) return 'inspector';
