...
class InspectorWorker {
  ...
  async classifySignal(context, analysis) {
    return {
      ...
      agentRole: this.determineAgentRole(context),
      escalationLevel: this.determineEscalationLevel(context),
  ...
  determineEscalationLevel(context) {
    const urgency = this.calculateUrgency(context);
