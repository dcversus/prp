...
export class ActionSuggestionEngine extends EventEmitter {
  private readonly historicalData = new Map<string, HistoricalAction[]>();
  private readonly actionTemplates = new Map<string, ActionTemplate>();
  private readonly successPredictor: SuccessPredictor;
  ...
  async generateSuggestions(
    ...
    try {
      logger.info(
        ...
        `Generating suggestions for signal ${classification.signalId}`,
      );
      // Step 1: Analyze action requirements
      const actionRequirements = this.analyzeActionRequirements(classification, context);
      // Step 2: Generate base suggestions from templates
      const templateSuggestions = await this.generateFromTemplates(
        classification,
        actionRequirements,
      );
      ...
      logger.info('ActionSuggestionEngine', `Generated ${finalSuggestions.length} suggestions`, {
        processingTime,
        averageConfidence:
          finalSuggestions.reduce((sum, s) => sum + s.metadata.confidence, 0) /
   ...
   */
  private analyzeActionRequirements(
    classification: EnhancedSignalClassification,
    context: ProcessingContext,
  ): ActionRequirements {
  ...
  private async generateFromTemplates(
    ...
    const applicableTemplates = Array.from(this.actionTemplates.values())
      .filter((template) => {
        ...
        return (
          template.applicableSignals.includes(classification.category) ||
    ...
    return applicableTemplates.map((template) =>
      this.createSuggestionFromTemplate(template, classification, requirements),
    );
  }
  /**
  ...
  private async generateFromHistory(
    ...
    const successfulActions = historicalActions
      ...
      .slice(0, 3);
    return successfulActions.map((action) =>
      this.createSuggestionFromHistory(action, classification, context),
    );
  }
  /**
  ...
  private initializeActionTemplates(): void {
    ...
    this.actionTemplates.set('bug-fix', {
      ...
      successRate: 0.78,
      priority: ActionPriority._CRITICAL,
    });
    // Testing template
    ...
    this.actionTemplates.set('research', {
      ...
      successRate: 0.88,
      priority: ActionPriority._MEDIUM,
    });
    // Deployment template
  ...
  private identifyDependencies(classification: EnhancedSignalClassification): string[] {
    ...
    if (
      structuredDeps &&
      typeof structuredDeps === 'object' &&
      'signalDependencies' in structuredDeps
    ) {
...
Provide 3-5 specific, actionable suggestions with details on implementation, requirements, and expected outcomes.`;
  ...
  private parseLLMSuggestions(
    _response: LLMSuggestionResponse,
    _classification: EnhancedSignalClassification,
    _context: ProcessingContext,
  ): EnhancedActionSuggestion[] {
  ...
  private createSuggestionFromHistory(
  ...
  private async findSuitableAgents(
    suggestion: EnhancedActionSuggestion,
  ...
  private async tailorSuggestionForAgent(
    _suggestion: EnhancedActionSuggestion,
  ...
  private calculateSuggestionScore(
    ...
    const complexityDiff = Math.abs(
      suggestion.capabilities.maxComplexity -
        this.convertComplexityToNumber(classification.complexity),
    );
  ...
  private mapStringToActionType(action: string): ActionType {
    const mapping: Record<string, ActionType> = {
      'dev-progress': ActionType._REVIEW,
      'bug-fix': ActionType._DEBUG,
      testing: ActionType._TEST,
      research: ActionType._RESEARCH,
      deployment: ActionType._DEPLOY,
      default: ActionType._ANALYZE,
    };
    return mapping[action] ?? ActionType._ANALYZE;
  }
  private mapUrgencyToPriority(urgency: string): ActionPriority {
    const mapping: Record<string, ActionPriority> = {
      immediate: ActionPriority._CRITICAL,
      high: ActionPriority._HIGH,
      medium: ActionPriority._MEDIUM,
      low: ActionPriority._LOW,
      background: ActionPriority._BACKGROUND,
    };
    return mapping[urgency] ?? ActionPriority._MEDIUM;
  }
}
// Supporting classes
