...
export class IntelligentPayloadGenerator extends EventEmitter {
  ...
  private contentPrioritization: ContentPrioritizer;
  ...
  async generatePayload(
    ...
    try {
      logger.info('IntelligentPayloadGenerator', `Starting payload generation for signal ${classification.signalId}`);
      ...
      logger.error('IntelligentPayloadGenerator', 'Payload generation failed', error instanceof Error ? error : new Error(errorMessage));
  ...
  private async analyzeContent(
    ...
    const criticalSections = sections.filter((s) => s.priority === ContentPriority.CRITICAL).map((s) => s.id);
    const optionalSections = sections.filter((s) => s.priority === ContentPriority.BACKGROUND).map((s) => s.id);
  ...
  private async createPayloadSection(
    ...
    try {
      ...
      logger.warn('IntelligentPayloadGenerator', `Failed to create section ${section.id}`, error as Record<string, unknown>);
      return null;
    }
  }
  ...
  private async compressSection(
    ...
    const applicableStrategies = Array.from(this.compressionStrategies.values())
      ...
      .sort((a: CompressionStrategy, b: CompressionStrategy) => b.compressionRatio - a.compressionRatio);
    ...
    for (const strategy of applicableStrategies) {
      ...
      try {
        ...
        logger.debug('IntelligentPayloadGenerator', `Applied compression strategy ${strategy.name} to section ${section.id}`);

      } catch (error) {
        logger.warn('IntelligentPayloadGenerator', `Compression strategy ${strategy.name} failed for section ${section.id}`, error as Record<string, unknown>);
      }
    }
  ...
  private async finalizePayload(
    ...
    const totalTokens = sections.reduce((sum: number, section: PayloadSection) => {
      ...
      const estimatedTokens = typeof actualTokens === 'number' ? actualTokens : TokenCounter.estimateTokensFromObject(section.content);
  ...
  private initializeCompressionStrategies(): void {
    ...
    this.compressionStrategies.set('key_filtering', {
      ...
      applicability: (content: unknown) => typeof content === 'object' && content !== null && !Array.isArray(content),
      compress: async (content: unknown) => this.filterObjectKeys(content as Record<string, unknown>),
      qualityImpact: 0.4,
  ...
  private filterRelevantAgents(agents: AgentStatusInfo[], classification: EnhancedSignalClassification): AgentStatusInfo[] {
    // Filter agents based on role relevance and availability
    return agents.filter((agent) =>
      agent.status === 'active' ||
      agent.type === classification.primary
  ...
  private calculateTargetTokens(section: ContentAnalysis['sections'][0], totalTarget: number): number {
  ...
  private convertClassificationForPayload(classification: EnhancedSignalClassification): SignalClassification {
    return {
  ...
  private validatePayload(payload: OptimizedInspectorPayload): void {
    if (payload.estimatedTokens > payload.targetSize * 1.1) { // Allow 10% tolerance
      logger.warn('IntelligentPayloadGenerator', `Payload exceeds target size: ${payload.estimatedTokens} > ${payload.targetSize}`);
    }
...
class TokenOptimizer {
  optimize(prioritizedContent: PrioritizedContent[], options: Record<string, unknown>): TokenAllocation {
    // Simple optimization - real implementation would use advanced algorithms
    const totalBudget = (options.totalBudget as number) * (1 - ((options.reserveForCompression as number) || 0.1));

    const allocations = prioritizedContent.map((pc) => ({
      sectionId: pc.section.id,
      allocatedTokens: Math.min(pc.targetTokens, Math.floor(totalBudget / prioritizedContent.length))
    }));
