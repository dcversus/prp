...
export interface InspectorGuideline {
  ...
  schema?: any;          // Optional JSON schema from inspector.py
  tokenLimits: {
    maxInput: number;
    maxOutput: number;
  };
...
export class EnhancedGuidelineAdapter {
  private guidelines: Map<string, InspectorGuideline> = new Map();
  private signalToGuidelineMap: Map<string, string[]> = new Map();
  ...
  async loadGuidelines(): Promise<GuidelineLoadResult> {
    ...
    try {
      ...
      logger.error('EnhancedGuidelineAdapter', 'Failed to load guidelines', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  private async loadGuidelinesFromCategory(
    ...
    try {
      ...
      logger.error('EnhancedGuidelineAdapter', `Failed to load category: ${category}`, error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
   ...
   */
  private async loadGuidelineFromPath(
    guidelineId: string,
    category: string,
    ...
    if (existsSync(inspectorPyPath)) {
      try {
        ...
        logger.warn('EnhancedGuidelineAdapter', `Failed to parse schema from inspector.py: ${guidelineId}`, {
          error: error instanceof Error ? error.message : String(error)
        });
      }
    }
    ...
    const guideline: InspectorGuideline = {
      id: guidelineId,
      category,
      name: metadata.name || guidelineId,
      ...
      schema,
      tokenLimits: {
        maxInput: metadata.maxInputTokens || 35000,
        maxOutput: metadata.maxOutputTokens || 40000
      },
   ...
   */
  private parseMarkdownMetadata(content: string): Record<string, any> {
   ...
   */
  private parsePythonSchema(content: string): any {
    try {
  ...
  private buildSignalMapping(): void {
    ...
    for (const [guidelineId, guideline] of this.guidelines) {
      ...
      for (const pattern of guideline.signalPatterns) {
        ...
        this.signalToGuidelineMap.get(normalizedPattern)!.push(guidelineId);
      }
    }

    logger.info('EnhancedGuidelineAdapter', 'Built signal mapping', {
      totalSignals: this.signalToGuidelineMap.size,
      totalMappings: Array.from(this.signalToGuidelineMap.values())
        .reduce((sum, guidelines) => sum + guidelines.length, 0)
    });
  }
  ...
  async getGuidelineForSignal(signal: Signal): Promise<string | null> {
    ...
    if (exactMatches && exactMatches.length > 0) {
      ...
      const sortedGuidelines = exactMatches
        ...
        .filter(Boolean)
        .sort((a, b) => {
          const aPriority = a?.signalPatterns.find(p => p.pattern.toLowerCase() === signalType)?.priority || 0;
          const bPriority = b?.signalPatterns.find(p => p.pattern.toLowerCase() === signalType)?.priority || 0;
      ...
      if (sortedGuidelines.length > 0) {
        ...
        logger.debug('EnhancedGuidelineAdapter', `Found exact match guideline: ${guideline.id}`, {
          ...
          priority: guideline.signalPatterns.find(p => p.pattern.toLowerCase() === signalType)?.priority
        });
    ...
    for (const [pattern, guidelineIds] of this.signalToGuidelineMap) {
      if (signalType.includes(pattern) || pattern.includes(signalType)) {
        const sortedGuidelines = guidelineIds
          ...
          .filter(Boolean)
          .sort((a, b) => {
            const aPriority = a?.signalPatterns.find(p => p.pattern.toLowerCase() === pattern)?.priority || 0;
            const bPriority = b?.signalPatterns.find(p => p.pattern.toLowerCase() === pattern)?.priority || 0;
            return bPriority - aPriority;
          });
        ...
        if (sortedGuidelines.length > 0) {
          ...
          logger.debug('EnhancedGuidelineAdapter', `Found partial match guideline: ${guideline.id}`, {
            signalType,
            pattern,
            guidelineId: guideline.id
          });
