...
import { EnhancedGuidelineAdapter } from './enhanced-guideline-adapter';

import type { Signal } from '../shared/types';
import type { EnhancedSignal } from '../shared/types/signals';
...
export class FIFOInspector extends EventEmitter {
  private readonly config: FIFOInspectorConfig;
  private readonly guidelineAdapter: EnhancedGuidelineAdapter;
  ...
  private readonly cache = new Map<string, InspectorAnalysisResult>();
  ...
  constructor(config?: Partial<FIFOInspectorConfig>) {
    ...
    this.guidelineAdapter = new EnhancedGuidelineAdapter();
  ...
  private setupPipelineListeners(): void {
    ...
    this.pipeline.on('inspector:processed', (data) => {
      logger.debug('FIFOInspector', 'Pipeline processed inspector signal', data);
      this.emit('pipeline:processed', data);
    });

    this.pipeline.on('orchestrator:queued', (data) => {
      logger.debug('FIFOInspector', 'Pipeline queued orchestrator signal', data);
      this.emit('pipeline:orchestrator:queued', data);
    });

    this.pipeline.on('error', (error) => {
      logger.error('FIFOInspector', 'Pipeline error', error);
  ...
  private async processSignal(queueItem: QueueItem): Promise<void> {
    ...
    const requestTimeout = setTimeout(async () => {
      await this.removeFromProcessing(id, 'Request timeout');
    ...
    try {
      ...
      if (this.config.features.enableCache) {
        ...
        if (cached) {
          await this.handleResult(id, cached);
          return;
        }
      }
      ...
      await this.handleResult(id, validatedResult);
    } catch (error) {
      logger.error(
        'FIFOInspector',
        `Signal processing failed: ${signal.type}`,
        error instanceof Error ? error : new Error(String(error)),
      );
      // Create error result
      ...
      await this.handleResult(id, errorResult);
    }
  }
  /**
   * Prepare analysis components within token limits
  ...
  private async prepareAnalysisComponents(
    ...
    tokenDistribution.total =
      tokenDistribution.base + tokenDistribution.guideline + tokenDistribution.context;
    // Verify within inspector cap
    if (tokenDistribution.total > this.config.tokenLimits.inspectorCap) {
      logger.warn(
        'FIFOInspector',
        `Prompt exceeds inspector cap: ${tokenDistribution.total}/${this.config.tokenLimits.inspectorCap}`,
      );
...
IMPORTANT: Ensure all required fields are included and values are properly typed. The deadline must be a valid ISO date string.`;
   ...
   * Execute LLM analysis with structured output
   */
  private async executeLLMAnalysis(prompt: string, signal: Signal): Promise<{
    classification: {
      category: string;
      subcategory: string;
      priority: number;
  ...
  private async validateAndStructureResult(
    ...
    const structuredResult: InspectorAnalysisResult = {
      ...
      classification: {
        ...
        dependencies: Array.isArray(result.classification.dependencies)
          ? result.classification.dependencies
          : [],
   ...
   */
  private async handleResult(queueId: string, result: InspectorAnalysisResult): Promise<void> {
    // Clear timeout
    ...
    this.metrics.averageProcessingTime =
      ...
      this.metrics.totalProcessed;
    // Create enhanced signal for pipeline
    const enhancedSignal: EnhancedSignal = {
      ...
      metadata: {
        ...
        worktree: (signal.data as any)?.worktree as string || '',
      },
    ...
    // Emit to unified pipeline
    try {
      await this.pipeline.processSignal(enhancedSignal);
      logger.debug('FIFOInspector', `Signal emitted to pipeline: ${result.signalId}`);
    } catch (pipelineError) {
      logger.error('FIFOInspector', 'Failed to emit signal to pipeline', pipelineError instanceof Error ? pipelineError : new Error(String(pipelineError)));
    }
   ...
   */
  private async removeFromProcessing(queueId: string, reason: string): Promise<void> {
    ...
    if (item) {
      ...
      await this.handleResult(queueId, errorResult);
    }
  }
  /**
