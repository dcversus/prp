...
import type { AgentConfig, AgentType, AgentRole } from './agent-config';
import type { DiscoveredAgent } from './agent-discovery';

// Define local ValidationResult interface for spawn validation
interface SpawnValidationError {
  field: string;
  message: string;
  code: string;
...
interface SpawnValidationResult {
  ...
  estimatedCosts: {
    perRequest: { min: number; max: number; average: number };
    perDay: { min: number; max: number; average: number };
    perMonth: { min: number; max: number; average: number };
    currency: string;
  };
  resourceUsage: {
    memory: { min: number; max: number; recommended: number };
    cpu: { min: number; max: number; recommended: number };
    storage: { temp: number; persistent: number };
    network: { bandwidth: number; requests: number };
  };
}
...
export class AgentSpawner extends EventEmitter {
  ...
  private maxConcurrentSpawns = 10;
  private currentSpawns = 0;
  ...
  async spawnAgent(request: SpawnRequest): Promise<SpawnResult> {
    ...
    try {
      ...
      if (request.role) {
        agentConfig.preferredRole = request.role;
      }
      if (request.requirements.maxCost) {
        agentConfig.maxCost = request.requirements.maxCost;
      }
      const discoveredAgent = await this.discovery.findBestAgent(agentConfig);
      if (!discoveredAgent) {
        return {
          success: false,
          error: {
  ...
  private async validateSpawnRequest(request: SpawnRequest): Promise<SpawnValidationResult> {
    // Basic validation
  ...
  private async doSpawnAgent(
    ...
    if (request.options.ttl !== undefined) {
      spawnedAgent.ttl = request.options.ttl;
    }
    try {
  ...
  private async performCleanup(): Promise<void> {
    ...
    for (const [agentId, agent] of Array.from(this.pool.agents.entries())) {
