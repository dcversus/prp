...
export class ConfigurationManager {
  ...
  constructor(private readonly _cwd: string = process.cwd()) {
  ...
  async load(configPath?: string): Promise<PRPConfig> {
    ...
    if (userConfigPath) {
      try {
        ...
        logger.debug('config', 'ConfigurationManager', `Loaded user configuration from: ${userConfigPath}`);
      } catch (error) {
        logger.warn(
          'config',
          'ConfigurationManager',
          `Failed to load user config from ${userConfigPath}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
    ...
    if (projectUserConfigPath) {
      try {
        ...
        logger.debug('config', 'ConfigurationManager', `Loaded project user configuration from: ${projectUserConfigPath}`);
      } catch (error) {
        logger.warn(
          'config',
          'ConfigurationManager',
          `Failed to load project user config from ${projectUserConfigPath}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
    ...
    if (targetPath) {
      try {
        ...
        logger.debug('config', 'ConfigurationManager', `Loaded project configuration from: ${targetPath}`);
      } catch (error) {
        logger.warn(
          'config',
          'ConfigurationManager',
          `Failed to load project config from ${targetPath}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    } else {
      logger.debug('config', 'ConfigurationManager', 'No project configuration file found, using user config and defaults');
    }
    ...
    */
    ...
    logger.info('config', 'ConfigurationManager', `Configuration loaded successfully from sources: ${this.loadedSources.join(', ')}`);
    return this.config;
  }
  /**
  ...
  async save(config: PRPConfig, configPath?: string): Promise<void> {
    ...
    */
    try {
      ...
      logger.info('config', 'ConfigurationManager', `Configuration saved to: ${targetPath}`);
    } catch (error) {
  ...
  applyCLIOverrides(overrides: Partial<PRPConfig>): void {
    ...
    logger.debug('config', 'ConfigurationManager', 'Applied CLI overrides to configuration');
  }
  /**
    ...
    try {
      ...
      return {
        ...
        errors: schemaResult.errors.map(
          (err) =>
            `${err.field}: ${err.message}${err.allowedValues ? ` (allowed: ${err.allowedValues.join(', ')})` : ''}`,
        ),
        warnings: schemaResult.warnings.map(
          (warn) =>
            `${warn.field}: ${warn.message}${warn.suggestion ? ` (suggestion: ${warn.suggestion})` : ''}`,
        ),
      };
    } catch (error) {
      logger.error(
  ...
  private findProjectConfigFile(): string | undefined {
    for (const configPath of CONFIG_PATHS) {
      const fullPath = path.join(this._cwd, configPath);
  ...
  private async readConfigFile(filePath: string): Promise<Partial<PRPConfig>> {
    ...
    try {
      ...
      if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
        throw new ConfigurationError(
          `Configuration file must contain an object, got ${typeof parsed}`,
        );
      }
  ...
  getSummary(): Record<string, unknown> {
    ...
    return {
      ...
      features: {
        ...
        quality: (this.config.settings.quality as { linting: { enabled: boolean } }).linting
          .enabled,
