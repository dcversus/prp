...
import type {
  TaskManager,
  TaskDefinition,
  TaskResult} from '../shared/tasks';
import type { ChainOfThought, CoTStep, CoTContext, DecisionRecord } from './types';
import type { Signal, InspectorPayload } from '../shared/types';
...
export class CoTProcessor {
  private readonly processingHistory = new Map<string, ChainOfThought>();
  ...
  async generateCoT(
    ...
    try {
      ...
      const processingContext: CoTContext = {
        ...
        availableAgents: (context.availableAgents ?? []).map((agent) =>
          typeof agent === 'string' ? agent : (agent.id ?? 'unknown'),
        ),
  ...
  async createTasksFromCoTDecision(
    ...
    if (!this.taskManager) {
      logger.warn(
        'orchestrator',
        'cot-processor',
        'No task manager available, cannot create tasks from CoT',
      );
      return [];
    }
    ...
    try {
      ...
      logger.info(
        'orchestrator',
        'cot-processor',
        `Created ${createdTasks.length} tasks from CoT decision`,
        {
          cotId: cot.id,
        },
      );
      ...
      logger.error(
        'orchestrator',
        'cot-processor',
        'Failed to create tasks from CoT decision',
   ...
   * Get CoT history
   */
  getCoTHistory(limit = 50): ChainOfThought[] {
   ...
   * Generate reasoning steps
   */
  private async generateReasoningSteps(signal: Signal, context: CoTContext): Promise<CoTStep[]> {
    const steps: CoTStep[] = [];
...
- Potential Issues: ${this.identifyPotentialIssues(signal, context)}`;
    return {
      ...
      reasoning:
        'Double-checking the decision for potential problems and ensuring it aligns with goals',
      confidence: 0.85,
      timestamp: new Date(),
    };
  }
  ...
  private calculateConfidence(steps: CoTStep[], decision: ChainOfThought['decision']): number {
    const stepConfidence =
      steps.reduce((sum, step) => sum + (step.confidence ?? 0), 0) / steps.length;
  ...
  private generateOptions(signal: Signal): string[] {
    ...
    switch (signal.type) {
      ...
      case 'Qb':
        return ['Analyze bug report', 'Reproduce issue', 'Fix bug', 'Create regression tests'];
      default:
        return baseOptions;
    }
  }
  ...
  private identifyCompleted(_signal: Signal, context: CoTContext): string[] {
    ...
    if (context.previousDecisions.length > 0) {
      ...
      for (const decision of recentDecisions) {
        completed.push(
          `Previous decision: ${decision.decision?.type ?? 'unknown type'} for PRP: ${decision.payload?.id ?? 'unknown'}`,
        );
      }
    }
    return completed;
   ...
   */
  private parseActionToTaskInstruction(action: {
    type?: string;
    task?: string;
    signalType?: string;
    data?: {
    ...
    try {
      ...
      logger.warn(
        'orchestrator',
        'cot-processor',
        'Failed to parse action to task instruction',
        error instanceof Error ? error : new Error(String(error)),
      );
      return null;
    }
  ...
  private formatTaskResults(results: TaskResult[]): string {
    ...
    const resultsList = results
      .map((result, index) => {
        ...
        return (
          `${index + 1}. ${status} Task ${result.taskId} (${duration}) - ${quality}\n` +
  ...
  private calculateResultsConfidence(results: TaskResult[]): number {
    ...
    const avgQuality =
      results.reduce((sum, result) => sum + result.quality.score, 0) / results.length;
    const successRate =
      results.filter((r) => r.outcome === TaskOutcome.SUCCESS).length / results.length;
  ...
  private updateDecisionWithResults(cot: ChainOfThought, results: TaskResult[]): void {
    ...
    (cot.decision as any).taskResults = {
