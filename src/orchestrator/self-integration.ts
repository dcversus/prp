...
export class SelfIntegration {
  private readonly selfConfig?: SelfConfig;
  ...
  async initialize(): Promise<SelfData> {
    ...
    if (this.initialized) {
      ...
      if (existing) {
        logger.debug('orchestrator', 'initialize', 'Using existing self identity', {
    ...
    if (existingSelf) {
      ...
      logger.info('orchestrator', 'initialize', 'Using stored self identity', {
        selfName: existingSelf.selfName,
      });
      return existingSelf;
    }
    ...
    if (!this.selfConfig?.enabled) {
      ...
      logger.info('orchestrator', 'initialize', 'Using default self identity', {
      ...
      try {
        ...
        logger.info('orchestrator', 'initialize', 'Self identity processed', {
          selfName: selfData.selfName,
        ...
        logger.error(
          'orchestrator',
          'initialize',
          'Failed to process self identity, using fallback',
        ...
        selfData = {
          ...
          selfSummary:
            this.extractSelfSummary(this.selfConfig.identity) ||
            'Managing autonomous development workflow with signal-driven orchestration',
          selfGoal:
            this.extractSelfGoal(this.selfConfig.identity) ||
  ...
  private async processSelfIdentity(selfInput: string): Promise<SelfData> {
    try {
      ...
      logger.error(
        'orchestrator',
        'processSelfIdentity',
        'Failed to process self identity',
        error instanceof Error ? error : new Error(String(error)),
      );
      // Fallback to basic extraction
   ...
   */
  private performBasicSelfReasoning(
    selfInput: string,
    ...
    const selfSummary =
      this.extractSelfSummary(selfInput) ||
      sharedContext ||
      'Managing autonomous development workflow with signal-driven orchestration';
    // Extract selfGoal from selfInput or combine PRP summaries
    const selfGoal =
      this.extractSelfGoal(selfInput) ||
      (prpSummary
        ? prpSummary.split(' ').join(' -- ANOTHER PRP -- ')
        : 'Orchestrate autonomous development across all PRPs');
    return {
      selfName,
      selfSummary,
  ...
  private extractSelfGoal(selfInput: string): string | null {
    ...
    const actionPattern =
      /(?:building|creating|developing|implementing|optimizing|managing|leading)\s+(.+?)(?:[.!?]|$)/i;
