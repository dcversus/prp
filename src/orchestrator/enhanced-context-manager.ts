...
// Dynamic context updater removed
...
import type { PRPFile, Signal } from '../shared/types';

const logger = createLayerLogger('orchestrator');
/**
 * Enhanced Context Manager - Unified context management system
 */
export class EnhancedContextManager implements IEnhancedContextManager {
  ...
  public readonly updater: null; // Dynamic context updater removed
  // Additional state for enhanced features
  private readonly activeSessions = new Map<string, ContextSession>();
  private readonly prpRegistry = new Map<string, PRPFile>();
  private readonly contexts = new Map<string, EnhancedContextSection>();
  ...
  constructor(contextLimits: Record<string, unknown> = {}) {
    ...
    this.updater = null; // Dynamic context updater removed
    // Initialize metrics
    this.contextMetrics = {
  ...
  async aggregateAndShareContexts(
    ...
    logger.debug(
      'aggregateAndShareContexts',
      `Aggregating ${prpIds.length} PRPs for ${agents.length} agents`,
    );
    try {
      ...
      logger.error(
        'aggregateAndShareContexts',
        'Failed to aggregate and share contexts',
  ...
  async extractAndSharePRPSection(
    ...
    logger.debug(
      'extractAndSharePRPSection',
      `Extracting ${sectionType} from ${prpId} for ${targetAgent}`,
    );
    try {
      ...
      logger.error(
        'extractAndSharePRPSection',
        'Failed to extract and share section',
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
    }
  ...
  async createContextSession(participants: string[]): Promise<ContextSession> {
    logger.debug(
      ...
      `Creating context session for ${participants.length} agents`,
    );
    try {
      ...
      logger.error(
        'createContextSession',
        'Failed to create context session',
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
    }
  ...
  async enableRealTimeSync(contextIds: string[]): Promise<void> {
    ...
    try {
      ...
      logger.error(
        'enableRealTimeSync',
        'Failed to enable real-time sync',
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
    }
  ...
  async registerPRP(prp: PRPFile): Promise<void> {
    ...
    try {
      ...
      logger.error(
        'registerPRP',
        `Failed to register PRP ${prp.name}`,
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
    }
  ...
  async buildEnhancedContext(
    ...
    try {
      // Build base context - simplified implementation
      const baseContext = {
        prompt: `Signal: ${signal.type}`,
        contextWindow: {
          total: 200000,
          used: 1000,
          available: 199000,
          sections: { agents: 0, shared: 0, prp: 1, tools: 0, system: 0 }
        },
        sections: []
      };
  ...
  getEnhancedContextSummary(): {
    ...
    const base = { status: 'active', sessions: this.activeSessions.size }; // Simplified base summary
  ...
  async handleSignal(signal: Signal): Promise<{
    ...
    try {
      ...
      if (typeof signal.data === 'object' && signal.data !== null) {
        const {targetAgents} = (signal.data as { targetAgents?: unknown[] });
      ...
      logger.error(
        'handleSignal',
        `Failed to process signal ${signal.type}`,
        error instanceof Error ? error : new Error(String(error)),
      );
      return {
        processed: false,
    ...
    try {
      ...
      for (const context of sharedContexts) {
        if (
          ...
          context.lastAccessed < new Date(Date.now() - 24 * 60 * 60 * 1000)
        ) {
      ...
      logger.error(
        ...
        'Failed to optimize context performance',
        error instanceof Error ? error : new Error(String(error)),
      );
      return {
        optimizationsApplied: [],
  ...
  async mergeContexts(contexts: EnhancedContextSection[]): Promise<EnhancedContextSection> {
    ...
    const merged: EnhancedContextSection = {
      ...
      relevanceScore: Math.max(
        ...contexts.map((c: EnhancedContextSection) => c.relevanceScore ?? 0),
      ),
