...
import { AgentRole, AgentCapabilities } from '../shared/types';
import { OrchestratorConfig, AgentTask, AgentSession } from './types';
...
export class AgentCommunication extends EventEmitter {
  ...
  private subAgents: Map<string, AgentSession> = new Map();
  private worktrees: Map<string, WorktreeConfig> = new Map();
  private messageThreads: Map<string, AgentMessage[]> = new Map();

  // Enhanced communication features
  private messageQueue: QueuedMessage[] = [];
  private topics: Map<string, MessageTopic> = new Map();
  private pendingRequests: Map<string, { resolve: Function; reject: Function; timeout: NodeJS.Timeout }> = new Map();
  private persistenceConfig: MessagePersistenceConfig;
  private enhancedStats: EnhancedCommunicationStats = {
    totalMessages: 0,
    messagesByType: {},
    messagesByAgent: {},
  ...
  constructor(private _config: OrchestratorConfig, persistenceConfig?: Partial<MessagePersistenceConfig>) {
    super();
  ...
  registerAgent(agentSession: AgentSession): void {
    ...
    this.stats.messagesByAgent[agentSession.id] = 0;
  ...
  unregisterAgent(agentId: string): void {
    ...
    if (agent) {
      ...
      this.stats.messagesByAgent[agentId] = 0;
   ...
   */
  async publishToTopic(topicName: string, message: Omit<AgentMessage, 'to'>): Promise<MessageDeliveryResult[]> {
    ...
    if (topic.persistMessages && this.persistenceConfig.enabled) {
      await this.persistMessage({
        ...message,
        to: topicName,
        metadata: { ...message.metadata, topic: topicName }
      });
    }
  ...
  async handleResponse(responseMessage: AgentMessage): Promise<void> {
    const correlationId = responseMessage.metadata.correlationId || responseMessage.metadata.replyTo;
    ...
    try {
      ...
      const task: AgentTask = {
        ...
        payload: {
          ...
          threadId: message.metadata.threadId || undefined
        },
        assignedAt: new Date(),
      ...
      return {
        ...
        response: result,
        executionTime: Date.now() - startTime,
        tokenUsage: task.tokenUsage?.total || undefined
      };
      ...
      logger.error('dispatchToPrimaryAgent', 'Agent task execution failed', new Error(errorMessage), errorDetails);
   ...
   */
  private async createSubAgent(config: SubAgentConfig, parentMessage: AgentMessage): Promise<AgentSession | null> {
    try {
      // Find base agent configuration for this role
      const baseAgent = Array.from(this.activeAgents.values())
        .find(agent => agent.agentConfig.role === config.role);
      ...
      logger.error('createSubAgent', 'Failed to create sub-agent', new Error(errorMessage), errorDetails);
      return null;
    }
  }
  ...
  private async cleanupSubAgent(subAgent: AgentSession): Promise<void> {
    try {
      ...
      logger.error('cleanupSubAgent', 'Failed to cleanup sub-agent', new Error(errorMessage), errorDetails);
    }
  }
  ...
  private findAgent(identifier: string): AgentSession | null {
    ...
    if (!agent) {
      agent = Array.from(this.activeAgents.values())
        .find(a => a.agentConfig.role === identifier);
    }
   ...
   */
  private updateStats(message: AgentMessage, results: MessageDeliveryResult[], executionTime: number): void {
    this.stats.totalMessages++;
    this.stats.messagesByType[message.type] = (this.stats.messagesByType[message.type] || 0) + 1;
    this.stats.messagesByAgent[message.to] = (this.stats.messagesByAgent[message.to] || 0) + 1;

    // Update average response time
    const totalTime = this.stats.averageResponseTime * (this.stats.totalMessages - 1) + executionTime;
    this.stats.averageResponseTime = totalTime / this.stats.totalMessages;

    // Count failed messages
    const failedCount = results.filter(r => !r.success).length;
    this.stats.failedMessages += failedCount;
  }
  ...
  private async filterAgentsByCapabilities(
    ...
    for (const agentId of agentIds) {
      ...
      if (match.matchScore > 0.5) { // 50% capability match threshold
  ...
  private calculateCapabilityMatch(
    ...
    for (const capability of requiredCapabilities) {
      if (agentCapabilities[capability as keyof AgentCapabilities]) {
  ...
  async sendStatusUpdate(protocol: StatusUpdateProtocol): Promise<MessageDeliveryResult[]> {
    ...
    await this.publishToTopic('status_updates', {
      ...message,
      to: 'status_updates'
    });
   ...
   */
  async sendCollaborationRequest(protocol: CollaborationRequestProtocol): Promise<MessageDeliveryResult[]> {
  ...
  async cleanup(): Promise<void> {
    ...
    // Cleanup worktrees
    for (const [worktreeId, config] of this.worktrees) {
  ...
  private async cleanupPersistence(): Promise<void> {
    try {
      ...
      const cutoffDate = new Date(Date.now() - this.persistenceConfig.retentionPeriod * 24 * 60 * 60 * 1000);
