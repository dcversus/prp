...
import type {
  ...
  ContextConflict,
  ConflictResolution,
  EnhancedContextSection} from './types';
...
export class ContextAggregator {
  ...
  private readonly conflictResolutionStrategies: Map<
    string,
    (conflicts: ContextConflict[]) => Promise<ConflictResolution[]>
  >;
  ...
  async aggregateContexts(
    ...
    logger.debug(
      'aggregateContexts',
      `Aggregating contexts for ${prpIds.length} PRPs using ${strategy} strategy`,
    );
    try {
  ...
  async mergeContexts(contexts: EnhancedContextSection[]): Promise<EnhancedContextSection> {
    ...
    // Merge each group
    for (const [, sections] of Array.from(groupedContexts.entries())) {
  ...
  async resolveConflicts(conflicts: ContextConflict[]): Promise<ConflictResolution[]> {
    ...
    for (const conflict of conflicts) {
      try {
        ...
        logger.warn(
          'resolveConflicts',
          `Failed to resolve conflict for section ${conflict.sectionId}`,
          {
  ...
  private initializeResolutionStrategies(): void {
    ...
    this.conflictResolutionStrategies.set(
      'permissions',
      this.resolvePermissionConflicts.bind(this),
    );
  }
  private async gatherAllSections(prpIds: string[]): Promise<EnhancedContextSection[]> {
    ...
    try {
      ...
      logger.debug(
        'gatherAllSections',
        `Gathered ${allSections.length} sections from ${prpIds.length} PRPs`,
      );
      return allSections;
    } catch (error) {
      logger.error(
        'gatherAllSections',
        'Failed to gather sections',
        error instanceof Error ? error : new Error(String(error)),
      );
      return [];
    }
  ...
  ): Promise<EnhancedContextSection[]> {
  ...
  private async tokenOptimizedStrategy(
    sections: EnhancedContextSection[],
  ): Promise<EnhancedContextSection[]> {
  ...
  private async relevanceScoredStrategy(
    sections: EnhancedContextSection[],
  ): Promise<EnhancedContextSection[]> {
  ...
  private detectConflicts(sections: EnhancedContextSection[]): ContextConflict[] {
    ...
    for (const [name, group] of Array.from(sectionsByName.entries())) {
      if (group.length > 1) {
  ...
  private async resolveContentConflicts(
  ...
  private async resolvePriorityConflicts(
    conflicts: ContextConflict[],
  ): Promise<ConflictResolution[]> {
    return conflicts.map((conflict) => ({
  ...
  private async mergeSectionGroup(
    sections: EnhancedContextSection[],
  ): Promise<EnhancedContextSection> {
