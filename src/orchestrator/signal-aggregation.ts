...
import type { EnhancedSignalData, SignalHistoryEntry } from './signal-router';
...
export class SignalAggregationSystem {
  private readonly agentNudge: ReturnType<typeof createAgentNudgeIntegration>;
  private readonly config: BulkDeliveryConfig;
  private readonly signalBatches = new Map<string, SignalBatch>();
  private readonly aggregationBuffers = new Map<string, Signal[]>();
   ...
   * Handle immediate delivery for critical signals
   */
  private handleImmediateDelivery(
    signal: Signal,
  ...
  private async deliverBatch(batch: SignalBatch): Promise<void> {
    ...
    try {
      ...
      if (batch.delivery.attempts >= batch.delivery.maxAttempts) {
        ...
        logger.error(
          'SignalAggregation',
          'Batch delivery failed permanently',
   ...
   * Find applicable multi-source aggregation rule
   */
  private findMultiSourceRule(
    signal: EnhancedSignalData,
    sourceSystem: string,
  ): MultiSourceAggregationRule | null {
    ...
    return (
      rules.find((rule) => {
        ...
        if (
          ...
          signal.routing.escalationLevel !== rule.conditions.escalationLevel
        ) {
          return false;
        }
        // Check PRP IDs
        const {prpId} = signal.context;
        ...
        if (rule.conditions.systemState) {
          const {systemState} = signal.context;
   ...
   */
  private handleImmediateEnhancedDelivery(
    signal: EnhancedSignalData,
   ...
   */
  private async enrichSignalContext(
    signal: EnhancedSignalData,
    config: {
      enabled: boolean;
      enrichRelatedSignals: boolean;
    ...
    if (config.enrichRelatedSignals && config.aggregationLevel === 'comprehensive') {
      ...
      signal.context.relatedSignals = [`related_${signal.id}_1`, `related_${signal.id}_2`];
    }
   ...
   */
  private isDuplicateEnhancedSignal(
    signal: EnhancedSignalData,
    bufferSignals: EnhancedSignalData[],
  ): boolean {
    return bufferSignals.some(
      (existingSignal) =>
        existingSignal.type === signal.type &&
        existingSignal.source === signal.source &&
        existingSignal.priority === signal.priority &&
   ...
   */
  private createEnhancedBatchMetadata(
    signals: EnhancedSignalData[],
    strategy: string,
  ): SignalAggregationResult['metadata'] {
    ...
    const enrichedSignals = signals.filter(
      (s) =>
        Object.keys(s.context.systemState).length > 0 || s.context.historicalContext.length > 0,
    ).length;
    const totalContextEntries = signals.reduce(
      (sum, s) => sum + s.context.historicalContext.length,
      0,
    );
    const averageHistoryEntries = signals.length > 0 ? totalContextEntries / signals.length : 0;
    // Create context summary
    const contextSummary = {
      systemStateEntries: signals.filter((s) => Object.keys(s.context.systemState).length > 0)
        .length,
      ...
      enrichmentLevels: [
        ...new Set(signals.map((s) => s.context.metadata.enrichmentLevel).filter(Boolean)),
      ],
    };
    return {
  ...
  private async deliverEnhancedBatch(batch: SignalAggregationResult): Promise<void> {
    if (batch.delivery.status !== 'pending') {
      logger.debug('SignalAggregation', 'Enhanced batch already processed', {
        batchId: batch.batchId,
      });
      return;
    }
    ...
    try {
      ...
      if (batch.delivery.attempts >= batch.delivery.maxAttempts) {
        ...
        logger.error(
          'SignalAggregation',
          'Enhanced batch delivery failed permanently',
          error instanceof Error ? error : new Error(errorMessage),
          {
            batchId: batch.batchId,
            attempts: batch.delivery.attempts,
  ...
  getEnhancedAggregationStats(): {
    ...
    const averageEnrichmentLevel =
      enrichmentLevels.length > 0
  ...
  private readonly enhancedSignalBatches = new Map<string, SignalAggregationResult>();
  private readonly enhancedAggregationBuffers = new Map<
    string,
    {
      signals: EnhancedSignalData[];
      rule: MultiSourceAggregationRule;
