...
import type { AgentTask, AgentSession, ActionResult, OrchestratorConfig } from './types';
import type { Signal, AgentRole, InspectorPayload } from '../shared/types';
...
export interface WorkflowDefinition {
  ...
  states: InternalWorkflowState[];
  transitions: InternalWorkflowTransition[];
  variables: InternalWorkflowVariable[];
  timeout?: number;
  retryPolicy?: InternalWorkflowRetryPolicy;
  metadata: {
    createdBy: string;
    createdAt: Date;
    tags: string[];
...
interface InternalWorkflowTrigger {
  id: string;
  type: 'signal' | 'manual' | 'schedule' | 'event' | 'condition';
  condition: string | ((_context: InternalWorkflowContext) => boolean);
...
interface InternalWorkflowState {
  ...
  entryActions?: InternalWorkflowAction[];
  exitActions?: InternalWorkflowAction[];
  timeout?: number;
  retryPolicy?: InternalWorkflowRetryPolicy;
  ...
  decisionOptions?: InternalWorkflowDecisionOption[];
  // Parallel-specific properties
  parallelBranches?: InternalWorkflowBranch[];
  // Wait-specific properties
  waitCondition?: string | ((_context: InternalWorkflowContext) => boolean);
  waitEvent?: string;
}
interface InternalWorkflowTransition {
  id: string;
  from: string; // state ID
  to: string; // state ID
  condition?: string | ((_context: InternalWorkflowContext) => boolean);
  actions?: InternalWorkflowAction[];
  priority: number;
  enabled: boolean;
  metadata?: Record<string, unknown>;
}
interface InternalWorkflowAction {
  id: string;
  type:
    | 'assign_agent'
    | 'send_signal'
    ...
    | 'escalate'
    | 'notify';
  parameters: Record<string, string | number | boolean | Record<string, unknown> | string[]>;
  retryPolicy?: InternalWorkflowRetryPolicy;
  timeout?: number;
  async: boolean;
}
interface InternalWorkflowVariable {
  name: string;
...
interface InternalWorkflowRetryPolicy {
  maxRetries: number;
  retryDelay: number; // milliseconds
  backoffMultiplier: number;
  retryOn: string[]; // error types
}
interface InternalWorkflowDecisionOption {
  id: string;
  name: string;
  condition?: string | ((_context: InternalWorkflowContext) => boolean);
  targetState: string;
...
interface InternalWorkflowBranch {
  id: string;
  name: string;
  condition?: string | ((_context: InternalWorkflowContext) => boolean);
...
export interface WorkflowExecution {
  ...
  currentState: string;
  history: InternalWorkflowExecutionHistory[];
  variables: Record<string, string | number | boolean | Record<string, unknown> | unknown[]>;
  context: InternalWorkflowContext;
  startTime: Date;
  endTime?: Date;
  error?: InternalWorkflowError;
  metadata: {
    triggeredBy: string;
    triggerData?: Record<string, string | number | boolean | Record<string, unknown>>;
...
export interface WorkflowContext {
  ...
  prpContext?: Record<string, string | number | boolean | Record<string, unknown>>;
  agentStates: Record<string, AgentSession>;
  globalVariables: Record<string, string | number | boolean | Record<string, unknown> | unknown[]>;
  systemMetrics: Record<string, number | string | boolean>;
  environment: 'development' | 'staging' | 'production';
  timestamp: Date;
  lastUpdated: Date;
}
interface InternalWorkflowContext {
  executionId: string;
  workflowId: string;
  signal?: Signal;
  inspectorPayload?: InspectorPayload;
...
interface InternalWorkflowExecutionHistory {
  timestamp: Date;
  fromState?: string;
  toState: string;
  action: string;
...
interface InternalWorkflowError {
  code: string;
  message: string;
...
interface InternalWorkflowExecutionStartedEvent {
  executionId: string;
  workflowId: string;
  triggerData: Record<string, string | number | boolean | Record<string, unknown>>;
  timestamp: Date;
}
interface InternalWorkflowStateChangedEvent {
  executionId: string;
  fromState: string;
  toState: string;
  context: InternalWorkflowContext;
...
interface InternalWorkflowActionExecutedEvent {
  executionId: string;
  actionId: string;
  actionType: string;
  result: ActionResult;
...
interface InternalWorkflowExecutionCompletedEvent {
  executionId: string;
  workflowId: string;
  status: 'completed' | 'failed' | 'cancelled';
  result: Record<string, string | number | boolean | Record<string, unknown> | unknown[]>;
  duration: number;
  timestamp: Date;
}
interface InternalWorkflowErrorEvent {
  executionId: string;
  error: InternalWorkflowError;
  timestamp: Date;
}
...
export class WorkflowEngine extends EventEmitter {
  ...
  private setupEventHandlers(): void {
    this.on('workflow:started', (event: InternalWorkflowExecutionStartedEvent) => this.handleWorkflowStarted(event));
    this.on('workflow:state_changed', (event: InternalWorkflowStateChangedEvent) => this.handleStateChanged(event));
    this.on('workflow:action_executed', (event: InternalWorkflowActionExecutedEvent) => this.handleActionExecuted(event));
    this.on('workflow:completed', (event: InternalWorkflowExecutionCompletedEvent) => this.handleWorkflowCompleted(event));
    this.on('workflow:error', (event: InternalWorkflowErrorEvent) => this.handleWorkflowError(event));
  }
  /**
  ...
  async start(): Promise<void> {
    ...
    this.isRunning = true;
    workflowLogger.debug('WorkflowEngine', 'Started successfully');
    workflowLogger.debug('WorkflowEngine', `Loaded ${this.workflows.size} workflow definitions`);
  ...
  async stop(): Promise<void> {
    ...
    workflowLogger.debug('WorkflowEngine', 'Stopped successfully');
  }
  /**
  ...
  registerWorkflow(workflow: WorkflowDefinition): void {
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Registered workflow: ${workflow.name} (${workflow.id})`,
    );
  }
  /**
  ...
  unregisterWorkflow(workflowId: string): boolean {
    ...
    if (deleted) {
      workflowLogger.debug(
        'WorkflowEngine',
        `[WorkflowEngine] Unregistered workflow: ${workflowId}`,
      );
    }
    return deleted;
  }
  ...
  async startWorkflow(
    ...
    const execution: WorkflowExecution = {
      ...
      history: [],
      variables: this.initializeVariables(workflow) as Record<
        string,
    ...
    this.emit('workflow:started', {
      ...
      triggerData: triggerData || {},
      timestamp: new Date(),
    } satisfies InternalWorkflowExecutionStartedEvent);
    // Start execution in background
    void setTimeout(() => this.executeWorkflow(executionId), 0);
    return executionId;
  }
  /**
   * Execute a workflow
  ...
  private async executeAction(
    ...
    try {
      ...
      this.emit('workflow:action_executed', {
        ...
        result,
        timestamp: new Date(),
      } satisfies InternalWorkflowActionExecutedEvent);
      return result;
    } catch (error) {
  ...
  private async executeTaskState(
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Created task ${task.id} for agent ${state.agentRole}`,
    );
  ...
  private async executeDecisionState(
    ...
    for (const option of state.decisionOptions) {
      ...
      const conditionMet =
        typeof option.condition === 'string'
  ...
  private async executeParallelState(
    ...
    if (
      (joinCondition === 'all' && successfulBranches === state.parallelBranches.length) ||
  ...
  private async executeWaitState(
    ...
    if (state.waitCondition) {
      const conditionMet =
        typeof state.waitCondition === 'string'
  ...
  private async executeEndState(
    ...
    this.emit('workflow:completed', {
      ...
      timestamp: new Date(),
    } satisfies InternalWorkflowExecutionCompletedEvent);
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Workflow execution ${execution.id} completed successfully`,
    );
  }
  private async executeErrorState(
    ...
    this.emit('workflow:completed', {
      ...
      result: {
        ...
        errorDetails: execution.error?.details,
      },
      duration: execution.endTime.getTime() - execution.startTime.getTime(),
      timestamp: new Date(),
    } satisfies InternalWorkflowExecutionCompletedEvent);
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Workflow execution ${execution.id} failed`,
    );
  }
  ...
  private async executeAssignAgentAction(
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Assigned agent ${agentId} (${agentRole}) to execution ${execution.id}`,
    );
    return {
      id: this.generateId(),
      actionId: action.id,
  ...
  private async executeSendSignalAction(
    execution: WorkflowExecution,
    action: InternalWorkflowAction,
  ): Promise<ActionResult> {
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Sent signal ${signal.id} (${signalType}) from execution ${execution.id}`,
    );
    return {
      id: this.generateId(),
      actionId: action.id,
  ...
  private async executeCommandAction(
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Executing command: ${command} ${args.join(' ')}`,
    );
  ...
  private async executeCreateTaskAction(
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Created task ${task.id} (${taskType}) for execution ${execution.id}`,
    );
    return {
      id: this.generateId(),
      actionId: action.id,
  ...
  private async executeUpdateContextAction(
    execution: WorkflowExecution,
    action: InternalWorkflowAction,
  ): Promise<ActionResult> {
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Updated context for execution ${execution.id}`,
    );
    return {
      id: this.generateId(),
      actionId: action.id,
  ...
  private async executeEscalateAction(
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Escalating execution ${execution.id} to ${escalateTo}: ${reason}`,
    );
  ...
  private async executeNotifyAction(
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Sending notification for execution ${execution.id}: ${message}`,
    );
  ...
  // ===== HELPER METHODS =====
  private findStartState(workflow: WorkflowDefinition): InternalWorkflowState {
  ...
  private evaluateCondition(condition: string, context: WorkflowContext): boolean {
    ...
    try {
      ...
      workflowLogger.error('WorkflowEngine', `Error evaluating condition: ${condition}`, error instanceof Error ? error : new Error(String(error)));
      return false;
    }
  }
  private async executeParallelBranch(
    execution: WorkflowExecution,
    branch: InternalWorkflowBranch,
    _workflow: WorkflowDefinition,
  ): Promise<string> {
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Executing parallel branch ${branch.id} for execution ${execution.id}`,
    );
    // Find the end state of the branch
    const lastStateId = branch.states[branch.states.length - 1];
    if (!lastStateId) {
      throw new Error(`Branch ${branch.id} has no states`);
    }
    return lastStateId;
  }
  private async transitionToState(
    ...
    this.emit('workflow:state_changed', {
      ...
      context: execution.context,
      timestamp: new Date(),
    } satisfies InternalWorkflowStateChangedEvent);
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Transitioned execution ${execution.id} from ${oldState} to ${toState}`,
    );
  }
  private async handleExecutionError(executionId: string, error: unknown): Promise<void> {
    ...
    const workflowError: InternalWorkflowError = {
    ...
    this.emit('workflow:error', {
      ...
      error: workflowError,
      timestamp: new Date(),
    } satisfies InternalWorkflowErrorEvent);
    workflowLogger.error('WorkflowEngine', `Execution ${executionId} failed:`, error instanceof Error ? error : new Error(String(error)));
  }
  private async resumePausedExecutions(): Promise<void> {
    const pausedExecutions = Array.from(this.executions.values()).filter(
      (exec) => exec.status === 'paused',
    );
    for (const execution of pausedExecutions) {
      workflowLogger.debug('WorkflowEngine', `[WorkflowEngine] Resuming execution ${execution.id}`);
  ...
  // ===== EVENT HANDLERS =====
  private handleWorkflowStarted(event: InternalWorkflowExecutionStartedEvent): void {
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Workflow execution started: ${event.executionId}`,
    );
  }
  private handleStateChanged(event: InternalWorkflowStateChangedEvent): void {
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] State changed: ${event.executionId} ${event.fromState} -> ${event.toState}`,
    );
  }
  private handleActionExecuted(event: InternalWorkflowActionExecutedEvent): void {
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Action executed: ${event.executionId} ${event.actionType} (${event.result.status})`,
    );
  }
  private handleWorkflowCompleted(event: InternalWorkflowExecutionCompletedEvent): void {
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Workflow completed: ${event.executionId} (${event.status})`,
    );
  }
  private handleWorkflowError(event: InternalWorkflowErrorEvent): void {
    workflowLogger.error('WorkflowEngine', `Workflow error: ${event.executionId}`, new Error(event.error.message));
  }
  // ===== BUILT-IN WORKFLOW DEFINITIONS =====
  /**
  ...
  private createCodeReviewWorkflow(): WorkflowDefinition {
    return {
      ...
      metadata: {
        ...
        requiredRoles: ['robo-aqa', 'robo-system-analyst'],
      },
    };
  }
  /**
    ...
    return {
      ...
      transitions: [
        { id: 'start_to_design', from: 'start', to: 'design_review', priority: 1, enabled: true },
        {
          id: 'design_to_setup',
          from: 'design_review',
          to: 'setup_development',
          priority: 1,
          enabled: true,
        },
        {
          id: 'setup_to_core',
          from: 'setup_development',
          to: 'implement_core',
          priority: 1,
          enabled: true,
        },
      ...
      metadata: {
        ...
        requiredRoles: ['robo-developer', 'robo-ui-designer', 'robo-aqa', 'robo-devops'],
      },
    };
  }
  /**
  ...
  private createBugFixWorkflow(): WorkflowDefinition {
    return {
      ...
      metadata: {
        ...
        requiredRoles: ['robo-system-analyst', 'robo-developer', 'robo-aqa', 'robo-devops'],
      },
    };
  }
  /**
  ...
  private createDeploymentWorkflow(): WorkflowDefinition {
    return {
          ...
          decisionOptions: [
            {
              ...
              condition:
                'context.globalVariables.stagingTestsPassed === true && context.globalVariables.stakeholderApproved === true',
            ...
            },
            {
              ...
              condition:
                'context.globalVariables.stagingTestsPassed === false || context.globalVariables.stakeholderApproved === false',
      ...
      transitions: [
        {
          id: 'start_to_checks',
          from: 'start',
          to: 'pre_deployment_checks',
          priority: 1,
          enabled: true,
        },
        {
          id: 'checks_to_build',
          from: 'pre_deployment_checks',
          to: 'build_application',
          priority: 1,
          enabled: true,
        },
        {
        ...
        {
          id: 'decision_to_rejected',
          from: 'approval_decision',
          to: 'deployment_rejected',
          priority: 2,
          enabled: true,
        },
        {
          id: 'production_to_verification',
          from: 'deploy_production',
          to: 'production_verification',
          priority: 1,
          enabled: true,
        },
        {
          id: 'verification_to_complete',
          from: 'production_verification',
          to: 'complete',
          priority: 1,
          enabled: true,
        },
        {
          id: 'rejected_to_complete',
          from: 'deployment_rejected',
          to: 'complete',
          priority: 1,
          enabled: true,
        },
      ...
      metadata: {
        ...
        requiredRoles: ['robo-devops', 'robo-aqa', 'robo-system-analyst'],
      },
    };
  }
  /**
      ...
      transitions: [
        {
          id: 'start_to_prepare',
          from: 'start',
          to: 'prepare_test_environment',
          priority: 1,
          enabled: true,
        },
        {
          id: 'prepare_to_unit',
          from: 'prepare_test_environment',
          to: 'unit_tests',
          priority: 1,
          enabled: true,
        },
        ...
        {
          id: 'e2e_to_performance',
          from: 'e2e_tests',
          to: 'performance_tests',
          priority: 1,
          enabled: true,
        },
        {
          id: 'performance_to_security',
        ...
        {
          id: 'compile_to_complete',
          from: 'compile_results',
          to: 'complete',
          priority: 1,
          enabled: true,
        },
  ...
  async cancelExecution(executionId: string, reason?: string): Promise<void> {
    ...
    workflowLogger.debug(
      'WorkflowEngine',
      `[WorkflowEngine] Cancelled execution ${executionId}: ${reason}`,
    );
  }
  /**
  ...
  async pauseExecution(executionId: string): Promise<void> {
    ...
    workflowLogger.debug('WorkflowEngine', `[WorkflowEngine] Paused execution ${executionId}`);
  }
  /**
  ...
  async resumeExecution(executionId: string): Promise<void> {
    ...
    workflowLogger.debug('WorkflowEngine', `[WorkflowEngine] Resumed execution ${executionId}`);
  }
  /**
    ...
    const averageExecutionTime =
      completedExecutions.length > 0
        ? completedExecutions.reduce((sum, exec) => {
            const duration = exec.endTime ? exec.endTime.getTime() - exec.startTime.getTime() : 0;
            return sum + duration;
