...
export class OrchestratorMessageHandlingGuidelines {
  ...
  private readonly messageQueue = new Map<string, AdminMessage>();
  private readonly readStatusTracker = new Map<
    string,
    {
      messageId: string;
      adminId: string;
  ...
  updateMessageReadStatus(
    ...
    if (!message) {
      logger.warn(
        'OrchestratorMessageHandler',
        'Attempted to update read status for unknown message',
        {
          messageId,
        },
      );
  ...
  private async processFollowUps(): Promise<void> {
    const sentMessages = Array.from(this.messageQueue.values()).filter(
      (msg) =>
    ...
    logger.info(
      'OrchestratorMessageHandler',
      `Processing follow-ups for ${sentMessages.length} messages`,
    );
  ...
  private shouldSendFollowUp(message: AdminMessage): boolean {
    ...
    const followUpInterval =
      message.metadata.followUpInterval ??
...
URGENT ACTION REQUIRED: ${message.actionRequired ?? 'Immediate admin attention required'}`;
  ...
  private isMessageExpired(message: AdminMessage): boolean {
    return message.metadata.expiresAt ? Date.now() > message.metadata.expiresAt.getTime() : false;
  }
