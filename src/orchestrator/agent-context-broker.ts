...
export class ContextBrokerImpl implements AgentContextBroker {
  ...
  private shareRecords: Map<string, ContextShareRecord> = new Map();
  private agentPermissions: Map<string, AgentPermissions> = new Map();
  private contextStore: Map<string, EnhancedContextSection> = new Map();
  ...
  async shareContext(
    ...
    try {
      ...
      logger.error('shareContext', `Failed to share context from ${fromAgent} to ${toAgent}`,
  ...
  async requestContext(agent: string, contextType: ContextType): Promise<EnhancedContextSection> {
    ...
    try {
      ...
      logger.error('requestContext', `Failed to provide context to agent ${agent}`,
        error instanceof Error ? error : new Error(String(error))
      );
      throw error;
    }
  ...
  async establishContextSession(participants: string[]): Promise<ContextSession> {
    logger.debug('establishContextSession', `Establishing context session for ${participants.length} agents`);
    ...
    try {
      ...
      logger.error('establishContextSession', 'Failed to establish context session',
        error instanceof Error ? error : new Error(String(error))
      );
      throw error;
    }
  ...
  async revokeContext(shareRecordId: string, revokingAgent: string): Promise<void> {
    ...
    try {
      ...
      logger.error('revokeContext', `Failed to revoke context share ${shareRecordId}`,
        error instanceof Error ? error : new Error(String(error))
      );
      throw error;
    }
  ...
  cleanup(): void {
    ...
    for (const [sessionId, session] of this.activeSessions) {
      ...
      if (hoursSinceActivity > 24) { // 24 hour timeout
  ...
  async broadcastUpdate(update: ContextUpdate): Promise<void> {
    ...
    try {
      ...
      const relevantSessions = Array.from(this.activeSessions.values())
        .filter(session => session.sharedContexts.has(update.contextId));
  ...
  private initializeDefaultPermissions(): void {
    ...
    const defaultPermissions: AgentPermissions = {
      agentId: 'default',
      canReceive: [
        ContextType.SHARED_MEMORY,
        ContextType.SIGNAL_HISTORY
      ],
      canShare: [
        ContextType.AGENT_STATUS
      ],
      trustedAgents: [],
      restrictions: []
    };
  ...
  private validateSharePermissions(fromAgent: string, toAgent: string, context: EnhancedContextSection): void {
    ...
    if (!fromPerms.canShare.includes(contextType)) {
      throw new Error(`Agent ${fromAgent} does not have permission to share ${contextType} contexts`);
    }
    ...
    if (!toPerms.canReceive.includes(contextType)) {
      throw new Error(`Agent ${toAgent} does not have permission to receive ${contextType} contexts`);
    }
  }
  ...
  private findContextsByType(contextType: ContextType, requestingAgent: string): EnhancedContextSection[] {
