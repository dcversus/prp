...
export type SignalHandler = (signal: SignalEvent) => Promise<void> | void;
...
export class SignalRouter extends EventEmitter {
  ...
  private readonly options: Required<SignalRouterOptions>;
  ...
  addRoute(pattern: RegExp, handler: SignalHandler, options: Partial<SignalRoute> = {}): string {
    ...
    if (this.options.enableLogging) {
      logger.info(
        'orchestrator',
        'SignalRouter',
        `Route added: ${routeId} (${route.metadata?.name ?? 'Unnamed'})`,
      );
    }
  ...
  removeRoute(routeId: string): boolean {
    ...
    if (this.options.enableLogging) {
      logger.info(
        'orchestrator',
        'SignalRouter',
        `Route removed: ${routeId} (${route.metadata?.name ?? 'Unnamed'})`,
      );
    }
    ...
    try {
        ...
        try {
          ...
          if (this.options.enableLogging) {
            logger.error(
              'orchestrator',
              'SignalRouter',
              `Handler error for route ${route.id}`,
      ...
      if (this.options.enableLogging && routesMatched > 0) {
        logger.info(
          'orchestrator',
          'SignalRouter',
          `Signal [${signal.signal}] routed to ${routesMatched} handlers in ${processingTime}ms`,
        );
      }
      ...
      if (this.options.enableLogging) {
        logger.error(
          'orchestrator',
          'SignalRouter',
          `Routing error for signal [${signal.signal}]`,
          error instanceof Error ? error : new Error(String(error)),
        );
      }
      return 0;
  ...
  private updateStats(): void {
    ...
    this.stats.activeRoutes = Array.from(this.routes.values()).filter(
      (route) => route.enabled,
    ).length;
  }
  /**
   ...
   */
  async routeEnhancedSignal(
    signal: EnhancedSignalData,
    agentCapabilities: AgentCapability[],
  ): Promise<SignalRoutingDecision> {
    const startTime = Date.now();
    try {
      ...
      if (this.options.enableLogging) {
        logger.info(
          'orchestrator',
          'SignalRouter',
          `Enhanced signal routed: ${signal.type} -> ${decision.targetAgent} in ${processingTime}ms`,
        );
      }
  ...
  private makeRoutingDecision(
    ...
    if (rule) {
      ...
      const targetAgent = availableAgents.find((agent) => rule.targetAgents.includes(agent.agent));
      if (targetAgent) {
        return {
