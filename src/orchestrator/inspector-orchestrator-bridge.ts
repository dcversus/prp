...
export class InspectorOrchestratorBridge extends EventEmitter {
  private config: Required<InspectorOrchestratorBridgeConfig>;
  private metrics: BridgeMetrics;
  private isRunning = false;
  private startTime?: Date;
  private healthCheckTimer?: NodeJS.Timeout;
  private pendingTasks: Map<string, TaskRequest> = new Map();
  private resultAggregations: Map<string, ResultAggregation> = new Map();
  ...
  async start(): Promise<void> {
    ...
    console.log('🌉 Inspector-Orchestrator Bridge started');
  }
  ...
  async stop(): Promise<void> {
    ...
    console.log('🌉 Inspector-Orchestrator Bridge stopped');
  }
  ...
  private async completeAggregation(aggregationId: string): Promise<void> {
    ...
    try {
      ...
      this.metrics.errorCount++;
      this.handleAggregationError(aggregation, error instanceof Error ? error : new Error(String(error)));
    } finally {
   ...
   * Forward payload to orchestrator
   */
  private async forwardPayloadToOrchestrator(payload: InspectorPayload, aggregationId: string): Promise<void> {
    if (!this.orchestrator) {
  ...
  private async processPendingTasks(): Promise<void> {
    ...
    for (const task of tasks) {
      try {
        // Log pending tasks
        console.log(`Pending task during shutdown: ${task.id} (${task.type})`);
      } catch (error) {
  ...
  async processDeadLetterQueue(): Promise<void> {
    ...
    console.log(`🔄 Retrying ${resultsToRetry.length} results from dead letter queue`);
