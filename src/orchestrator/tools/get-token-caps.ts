...
export const getTokenCapsTool: Tool = {
  ...
  name: 'get_token_caps',
  description:
    'Get current token limits and usage for inspector and orchestrator based on PRP-007 specifications',
  category: 'monitoring',
  enabled: true,
  parameters: {
    agentType: {
...
function getTokenCapsData(agentType?: 'inspector' | 'orchestrator' | 'all'): TokenCapsData {
  ...
  const systemCaps = {
    ...
    overallUsage:
      ((inspectorCaps.usage.current + orchestratorCaps.usage.current) /
        (inspectorCaps.total + orchestratorCaps.total)) *
      100, // ~14.2%
  };
