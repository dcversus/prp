...
type TokenCaps = Record<string, TokenCapsData>;
...
export const getTokenUsageTool: Tool = {
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.info(
        'get_token_usage',
        `Collected token usage data for ${typedUsageData.agents.length ?? 0} agents`,
      );
      return {
        success: true,
        data: {
...
export const setTokenLimitsTool: Tool = {
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.info(
        'set_token_limits',
        `Set token limits for ${typedParams.agentId ?? typedParams.agentType ?? 'default'}`,
      );
      return {
        success: true,
        data: {
...
export const getTokenProjectionsTool: Tool = {
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.info(
        'get_token_projections',
        `Generated ${typedParams.timeframe ?? 'daily'} token projections`,
      );
      return {
        success: true,
        data: {
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.info(
        ...
        `Analyzed token efficiency for ${typedEfficiencyData.analyzedAgents ?? 0} agents`,
      );
      return {
        success: true,
        data: {
...
export const configureTokenAlertsTool: Tool = {
  ...
  parameters: {
    agentId: {
      type: 'string',
      description:
        'Agent ID to configure alerts for (optional, configures default if not provided)',
      required: false,
    },
    alertType: {
      type: 'string',
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.info(
        'configure_token_alerts',
        `Configured ${typedParams.alertType} alert for ${typedParams.agentId ?? 'default'}`,
      );
      return {
        success: true,
        data: {
...
function calculateProjections(
  historicalData: HistoricalTokenUsage,
  params: GetTokenProjectionsParams,
): ProjectionResult {
  ...
  const avgUsage =
    usageValues.reduce((sum: number, val: number) => sum + val, 0) / usageValues.length;
  // Calculate trend
  const trend =
    usageValues.length >= 2
...
export const getTokenCapsTool: Tool = {
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.info(
        'get_token_caps',
        `Retrieved token caps for ${typedParams.agentType || 'all agents'}`,
      );
      return {
        success: true,
        data: {
...
export const setDynamicTokenCapsTool: Tool = {
  ...
  parameters: {
    ...
    scaleFactor: {
      type: 'number',
      description:
        'Scale factor for adjustment (1.0 = no change, >1.0 = increase, <1.0 = decrease)',
      required: false,
    },
    newLimits: {
      type: 'object',
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.info(
        'set_dynamic_token_caps',
        `Adjusted token caps using ${typedParams.adjustmentStrategy} strategy`,
      );
      return {
        success: true,
        data: {
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.info(
        ...
        `Allocated ${typedParams.totalBudget} tokens using ${typedParams.allocationMethod} method`,
      );
      return {
        success: true,
        data: {
      ...
      logger.error(
        'allocate_token_budget',
        'Failed to allocate token budget',
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
    }
...
async function getTokenCapsData(params: {
  ...
  const result: TokenCapsResult = {
    ...
    summary: {
      ...
      totalLimit: Object.values(filteredCaps).reduce(
        (sum: number, cap: TokenCapsData) => sum + cap.totalLimit,
        0,
      ),
      totalUsed: Object.values(filteredCaps).reduce(
        (sum: number, cap: TokenCapsData) => sum + cap.currentUsage,
        0,
      ),
      averageUsage: 0,
    },
  ...
  if (params.includeProjections) {
    result.projections = {
      next24Hours: {
        ...
        riskLevel:
          result.summary.averageUsage > 80
            ? 'high'
            : result.summary.averageUsage > 60
              ? 'medium'
              : 'low',
        recommendations:
          result.summary.averageUsage > 80
      ...
      nextWeek: {
        ...
        budgetRecommendations:
          result.summary.averageUsage > 70
        ...
        Object.entries(currentCaps.tokenCaps).forEach(([agentType, caps]: [string, unknown]) => {
          ...
          newCaps[agentType] = {
            ...
            allocation: Object.fromEntries(
              Object.entries(typedCaps.allocation).map(([key, value]) => [
                key,
                Math.floor(value * (params.scaleFactor ?? 1)),
              ]),
            ),
          };
          adjustmentLog.push(
            `${agentType}: ${typedCaps.totalLimit} -> ${newCaps[agentType]?.totalLimit} (${params.scaleFactor ?? 1}x)`,
          );
        });
      }
      break;
      ...
      Object.entries(currentCaps.tokenCaps).forEach(([agentType, caps]: [string, unknown]) => {
        ...
        newCaps[agentType] = {
          ...
          allocation: Object.fromEntries(
            Object.entries(typedCaps.allocation).map(([key, value]) => [
              key,
              Math.floor(value * adjustmentFactor),
            ]),
          ),
        };
        adjustmentLog.push(
          `${agentType}: ${typedCaps.totalLimit} -> ${newCaps[agentType]?.totalLimit} (${adjustmentFactor}x, usage: ${(usageRatio * 100).toFixed(1)}%)`,
        );
      });
      break;
    case 'cost_based':
      ...
      Object.entries(currentCaps.tokenCaps).forEach(([agentType, caps]: [string, unknown]) => {
        ...
        adjustmentLog.push(
          `${agentType}: ${typedCaps.totalLimit} -> ${newCaps[agentType]?.totalLimit} (${adjustmentFactor}x, est. cost: $${estimatedCost.toFixed(2)})`,
        );
      });
      break;
    default:
...
async function allocateTokenBudget(params: {
  ...
  switch (params.allocationMethod) {
    ...
    case 'priority_based': {
      ...
      agents.forEach((agent) => {
        allocations[agent] = Math.floor(
          availableBudget * priorityWeights[agent as keyof typeof priorityWeights],
        );
      });
      break;
    }
    case 'usage_based': {
      ...
      agents.forEach((agent) => {
        allocations[agent] = Math.floor(
          availableBudget * usageWeights[agent as keyof typeof usageWeights],
        );
      });
      break;
    }
    case 'performance_based': {
      ...
      const performanceWeights = { inspector: 0.45, orchestrator: 0.4, scanner: 0.15 };
      agents.forEach((agent) => {
        allocations[agent] = Math.floor(
          availableBudget * performanceWeights[agent as keyof typeof performanceWeights],
        );
      });
      break;
    }
    case 'custom': {
      ...
      const totalWeight = Object.values(params.agentWeights).reduce(
...
function generateBudgetRecommendations(
  ...
  if (maxAllocation > minAllocation * 3) {
    recommendations.push(
      'Consider rebalancing allocations - there is a significant disparity between agent budgets',
    );
  }
  ...
  agentAllocations.forEach(([agent, amount]) => {
    ...
    if (percentage > 50) {
      recommendations.push(
        `${agent} allocation is high (${percentage.toFixed(1)}%) - monitor usage closely`,
      );
    }
  });
