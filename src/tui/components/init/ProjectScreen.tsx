...
const ProjectScreen: React.FC<ProjectScreenProps> = ({
  ...
  const updateProjectPath = useCallback((projectName: string) => {
  ...
  const handleProjectPromptChange = useCallback((projectPrompt: string) => {
  ...
  return (
    <Box flexDirection="column" flexGrow={1}>
      ...
      <FieldText
        ...
        required={true}
        error={state.projectName.length > 0 && state.projectName.length < 3 ? 'Must be at least 3 characters long' : ''}
      />
      ...
      <Box flexDirection="column" marginTop={2}>
        ...
        <Box flexDirection="row">
          ...
          <Text color={(theme.colors as any).accentOrange ?? (theme.colors as any).orange ?? '#FF9A38'} bold>
            {state.projectPath ? state.projectPath.split('/').pop() : 'project-name'}
          </Text>
        </Box>
