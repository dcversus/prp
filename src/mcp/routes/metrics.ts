...
const gitStatusCheckDuration = new client.Histogram({
...
const gitBranchesCount = new client.Gauge({
  name: 'git_branches_count',
  help: 'Number of git branches being monitored',
  registers: [register]
});

const gitCommitsAheadBehind = new client.Gauge({
...
router.get('/', trackMetrics, async (req: Request, res: Response) => {
  try {
    ...
    const mcpServer = (req as any).mcpServer;
    ...
    if (mcpServer) {
      ...
      systemMemoryUsage
        .labels('rss')
        .set(status.server.memory.rss);
      systemMemoryUsage
        .labels('heap_used')
        .set(status.server.memory.heapUsed);
      systemMemoryUsage
        .labels('heap_total')
        .set(status.server.memory.heapTotal);
      systemMemoryUsage
        .labels('external')
        .set(status.server.memory.external);

      // Update scanner metrics if scanner is available
      const scanner = mcpServer.scanner;
      if (scanner && typeof scanner.getMetrics === 'function') {
        const scannerMetrics = scanner.getMetrics();

        // Scanner performance metrics
        if (scannerMetrics.scanMetrics) {
          scannerTotalScans.set(scannerMetrics.scanMetrics.totalScans || 0);
          ...
          scannerErrorCount.labels('general').inc(scannerMetrics.scanMetrics?.errors || 0);
        }

        // Worktree metrics
        if (scannerMetrics.worktrees) {
          const worktrees = scannerMetrics.worktrees;
          let cleanCount = 0, dirtyCount = 0, conflictCount = 0, divergedCount = 0;
          let totalPRPFiles = 0;
          let totalFilesScanned = 0;
          const fileChangeTypes: { [key: string]: number } = {};
          ...
          worktrees.forEach((wt: any) => {
            ...
            switch (wt.status) {
              ...
              case 'diverged': divergedCount++; break;
            }
