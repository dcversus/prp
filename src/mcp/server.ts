...
import { MCPAuth } from '../shared/mcp/auth';
...
import type {
  ...
  StreamRequest,
  MCPAgentStatus,
  AgentStatus} from './types';

const logger = createLayerLogger('mcp-server');

// Helper function to properly call logger.debug with component
const logDebug = (component: string, message: string, metadata?: Record<string, unknown>) => {
...
export class MCPServer {
  ...
  private readonly scanner: IScannerCore;
  ...
  constructor(config: MCPServerConfig) {
    ...
    if (!config.ssl && process.env['NODE_ENV'] === 'production') {
      throw new Error(
        'SSL is required for MCP server in production. Please provide SSL certificates or use reverse proxy.',
      );
    }
    this.app = express();
    ...
    try {
      const {AgentManager} = require('../orchestrator/agent-manager');
    ...
    try {
      const {ScannerCore} = require('../scanner/ScannerCore');
  ...
  private setupRoutes(): void {
    ...
    this.app.use(
      '/mcp/message',
      this.auth.getAuthMiddleware(['write']),
      messageRouter(this.auth, this.io),
    );
  ...
  private async handleOrchestratorMessage(
    ...
    logDebug('orchestrator', 'message received', {
      socketId: socket.id,
    ...
    switch (message.type) {
      ...
      case 'agent':
        ...
        await this.forwardToAgent(message.payload.agent || 'unknown', message);
        break;
      case 'system':
  ...
  private async forwardToOrchestrator(message: MCPOrchestratorMessage): Promise<void> {
    ...
    logDebug('orchestrator', 'forwarding message', { message });
  ...
  private async forwardToAgent(agentId: string, message: MCPOrchestratorMessage): Promise<void> {
    ...
    logDebug('agent', 'forwarding message', {
      agentId,
      message,
    });
  ...
  private async handleSystemCommand(message: MCPOrchestratorMessage): Promise<void> {
    // Handle system-level commands
    logDebug('system', 'handling command', { action: message.payload.action });
    switch (message.payload.action) {
      case 'shutdown':
        logDebug('system', 'graceful shutdown initiated via MCP', {});
        await this.gracefulShutdown();
        break;
      case 'restart':
        logDebug('system', 'restart requested via MCP', {});
        // Handle restart logic
        break;
      default:
        logDebug('system', 'unknown command', {
          action: message.payload.action,
        });
    }
  }
  private async streamOrchestratorOutput(
    socket: any,
    streamId: string,
    params: Record<string, unknown>,
  ): Promise<void> {
    // Simulate streaming orchestrator output
    const duration = (params.duration as number) || 10000; // 10 seconds default
  ...
  private async streamAgentOutput(
    ...
    const duration = (params.duration as number) || 5000; // 5 seconds default
  ...
  private async streamStatusUpdates(
    socket: any,
    streamId: string,
    params: Record<string, unknown>,
  ): Promise<void> {
    // Stream status updates
    const duration = (params.duration as number) || 15000; // 15 seconds default
  ...
  private async getStatus(): Promise<MCPStatus> {
    ...
    const totalClients = this.auth.getAllClients();
    // Get real agent data
  ...
  private async getAgentStatus(): Promise<{
    ...
    try {
      ...
      const agentStatuses = this.agentManager.getAllStatuses
        ? this.agentManager.getAllStatuses()
        : new Map();
      const activeAgentsCount = this.agentManager.getActiveAgentCount
        ? this.agentManager.getActiveAgentCount()
        : 0;
      const agentDetails = Array.from(agentStatuses.values()).map(
        (status: AgentStatus): MCPAgentStatus => ({
          id: status.agentId,
          name: status.config?.name || 'Unknown Agent',
          type: status.config?.role || 'agent',
          status: (status.isActive ? 'working' : 'idle') as
            | 'working'
            | 'idle'
            | 'error'
            | 'offline',
    ...
    try {
      if (!this.scanner) {
        ...
        try {
          ...
          const prpFiles = files.filter(
            (file: string) => file.endsWith('.md') && file.startsWith('PRP-'),
          );
          return {
  ...
  private setupGracefulShutdown(): void {
    const shutdown = async (signal: string) => {
      logDebug('system', 'starting graceful shutdown', { signal });
      // Stop accepting new connections
      this.server.close(() => {
        logDebug('system', 'HTTP server closed', {});
      });
      // Close all WebSocket connections
      this.io.close(() => {
        logDebug('system', 'WebSocket server closed', {});
      });
      // Cleanup resources
      setTimeout(() => {
        logDebug('system', 'graceful shutdown completed', {});
        process.exit(0);
      }, 5000);
    };
  ...
  private async gracefulShutdown(): Promise<void> {
    logDebug('system', 'initiating graceful shutdown from MCP command', {});
    ...
    // Close server
    this.server.close(() => {
      logDebug('system', 'server shutdown complete', {});
      process.exit(0);
    });
  }
  public start(): Promise<void> {
    return new Promise((resolve) => {
      this.server.listen(this.config.port, this.config.host, () => {
        logDebug('server', 'MCP server started', {
          host: this.config.host,
          port: this.config.port,
        });
        logDebug('server', 'SSL enabled', { enabled: this.config.ssl ? 'enabled' : 'disabled' });
        logDebug('server', 'Max connections', { maxConnections: this.config.maxConnections });
        logDebug('server', 'JWT expiration', { expiration: this.config.jwtExpiration });
        logDebug('server', 'CORS origins', { origins: this.config.corsOrigins.join(', ') });
        resolve();
      });
    });
  }
  public stop(): Promise<void> {
    return new Promise((resolve) => {
      this.server.close(() => {
        logDebug('server', 'MCP server stopped', {});
        resolve();
      });
    });
  }
  public getServerStatus(): MCPStatus {
    return {
      ...
      connections: {
        active: this.connections.size,
        total: this.auth.getAllClients().length,
