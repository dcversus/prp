...
export class LogsManager extends EventEmitter {
  ...
  private readonly logCache = new Map<string, LogEntry[]>();
  private sessionSummaries = new Map<string, SessionSummary>();
  private index = new Map<string, Set<string>>(); // Inverted index for search
  ...
  async searchLogs(query: LogSearchQuery): Promise<LogSearchResult> {
    ...
    for (const [cacheKey, logs] of Array.from(this.logCache.entries())) {
   ...
   */
  async getLastLogs(agentId?: string, tokenLimit = 2000): Promise<LogEntry[]> {
    const query: LogSearchQuery = {
      agentIds: agentId ? [agentId] : undefined,
  ...
  getActiveSessions(): SessionSummary[] {
    return Array.from(this.sessionSummaries.values()).filter(
  ...
  async cleanupOldLogs(): Promise<number> {
    ...
    for (const [sessionId, summary] of Array.from(this.sessionSummaries.entries())) {
  ...
  getStorageStats(): {
    ...
    let totalEntries = 0;
    for (const logs of Array.from(this.logCache.values())) {
      totalEntries += logs.length;
    }
    let indexSize = 0;
    for (const indexedEntries of Array.from(this.index.values())) {
    ...
    try {
      ...
      logger.warn('LogsManager', 'Failed to calculate disk usage', {
  ...
  private loadIndexes(): void {
    try {
      ...
      logger.warn('LogsManager', 'Failed to load indexes, starting fresh', {
        error: error instanceof Error ? error.message : String(error),
        stack: error instanceof Error ? error.stack : undefined,
      });
    }
  ...
  private matchesQuery(
    log: LogEntry,
    criteria: Record<string, Set<string>>,
    query: LogSearchQuery,
  ): boolean {
    // Check time range
    ...
    if (query.searchText) {
      ...
      const searchText =
        `${log.message} ${log.agentId || ''} ${log.signal || ''} ${(log.tags || []).join(' ')}`.toLowerCase();
  ...
  private async searchDiskFiles(query: LogSearchQuery): Promise<LogEntry[]> {
    ...
    for (const file of files) {
      try {
        ...
        logger.warn('LogsManager', `Failed to search disk file ${file}`, {
          error: error instanceof Error ? error.message : String(error),
          stack: error instanceof Error ? error.stack : undefined,
        });
      }
  ...
  private async flushCacheKey(cacheKey: string, logs: LogEntry[]): Promise<void> {
    try {
      ...
      logger.error(
        'LogsManager',
        `Failed to flush cache key ${cacheKey}`,
        error instanceof Error ? error : new Error(String(error)),
      );
    }
  }
  ...
  private async cleanupDiskFiles(cutoffDate: Date): Promise<number> {
    ...
    for (const file of files) {
      try {
        ...
        logger.warn('LogsManager', `Failed to clean up disk file ${file}`, {
          error: error instanceof Error ? error.message : String(error),
          stack: error instanceof Error ? error.stack : undefined,
        });
      }
