...
export class PRPContentTracker extends EventEmitter {
  ...
  private prpStates: Map<string, PRPState> = new Map();
  private signalDetector: (content: string) => DetectedSignal[];
  private cacheConfig: CacheConfig;
  private memoryCache: Map<string, { content: string; timestamp: Date; version: PRPVersion }> = new Map();
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info('PRPContentTracker', `✅ PRP content tracker initialized. Monitoring ${this.prpStates.size} PRPs`);
      ...
      logger.error('PRPContentTracker', 'Failed to initialize PRP content tracker', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async scanForChanges(): Promise<Array<{ prpPath: string; changes: ContentDiff }>> {
    ...
    for (const [prpPath, prpState] of this.prpStates.entries()) {
      ...
      try {
        ...
        logger.error('PRPContentTracker', 'Error scanning PRP', error instanceof Error ? error : new Error(String(error)), {
          prpPath,
   ...
   */
  getMonitoredPRPs(): Array<{ path: string; name: string; lastModified: Date; versionCount: number }> {
  ...
  getMetrics(): TrackerMetrics {
    return {
      ...this.metrics,
      totalVersions: Array.from(this.prpStates.values())
        .reduce((sum, state) => sum + state.versionHistory.length, 0),
      totalSignals: Array.from(this.prpStates.values())
        .reduce((sum, state) => sum + (state.currentVersion?.signals.length ?? 0), 0),
  ...
  async refreshPRP(prpPath: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error('PRPContentTracker', 'Failed to refresh PRP', error instanceof Error ? error : new Error(String(error)), {
        prpPath
      });
      return false;
    }
   ...
   */
  async searchPRPs(query: string, options: {
    ...
    for (const [prpPath, prpState] of this.prpStates.entries()) {
      ...
      const versionsToSearch = [prpState.currentVersion, ...prpState.versionHistory.slice(-5)]
        .filter(v => v !== null);
  ...
  private async discoverPRPFiles(): Promise<void> {
    try {
      ...
      logger.error('PRPContentTracker', 'Failed to discover PRP files', error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  private calculateDiff(previousVersion: PRPVersion, newVersion: PRPVersion): ContentDiff {
    ...
    const hasChanges = linesAdded > 0 || linesRemoved > 0 ||
                      signalsAdded.length > 0 || signalsRemoved.length > 0 ||
  ...
  private calculateAverageVersionSize(): number {
    const allVersions = Array.from(this.prpStates.values())
      .flatMap(state => state.versionHistory);
    ...
    try {
      ...
      if (data.cache) {
        for (const [key, entry] of Object.entries(data.cache)) {
          ...
          if (new Date(cacheEntry.timestamp).getTime() > TimeUtils.now().getTime() - this.cacheConfig.ttl) {
  ...
  private async persistCacheData(): Promise<void> {
    ...
    try {
      ...
      logger.error('PRPContentTracker', 'Failed to persist cache data', error instanceof Error ? error : new Error(String(error)));
    }
  }
}