...
export class MultiProviderTokenAccounting extends EventEmitter {
  ...
  constructor(persistPath: string = '.prp/multi-provider-token-accounting.json') {
    super();
  ...
  async initialize(): Promise<void> {
    ...
    try {
      logger.info('MultiProviderTokenAccounting', 'Initializing multi-provider token accounting...');
      ...
      logger.error('MultiProviderTokenAccounting', 'Failed to initialize', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  recordUsage(
    ...
    try {
      ...
      if (!detection.provider) {
        logger.warn('MultiProviderTokenAccounting', 'Could not detect provider from metadata', { agentId, operation });
        return;
      }

      const provider = this.providers.get(detection.provider);
      if (!provider?.enabled) {
        logger.warn('MultiProviderTokenAccounting', 'Provider not found or disabled', { provider: detection.provider });
        return;
      }
      ...
      if (cost > 0.10 || record.totalTokens > 10000) {
      ...
      logger.error('MultiProviderTokenAccounting', 'Error recording usage', error instanceof Error ? error : new Error(String(error)), {
        agentId,
        operation
      });
    }
  ...
  getProviderUsage(): ProviderUsage[] {
    ...
    for (const provider of this.providers.values()) {
      ...
      const dailyPercent = provider.rateLimits.tokensPerDay > 0
        ? (dailyUsage / provider.rateLimits.tokensPerDay) * 100
        : 0;
      const weeklyPercent = provider.rateLimits.tokensPerDay * 7 > 0
        ? (weeklyUsage / (provider.rateLimits.tokensPerDay * 7)) * 100
        : 0;
      const monthlyPercent = provider.rateLimits.tokensPerDay * 30 > 0
  ...
  getLimitPredictions(): LimitPrediction[] {
    ...
    for (const provider of this.providers.values()) {
      ...
      const averageHourlyUsage = hourlyUsage.reduce((sum, usage) => sum + usage, 0) / hourlyUsage.length;

      // Predict time to limit
      const dailyLimit = provider.rateLimits.tokensPerDay;
      const currentDailyUsage = this.calculateUsageInPeriod(providerRecords, TimeUtils.daysAgo(1), TimeUtils.now());

      const remainingDailyLimit = dailyLimit - currentDailyUsage;
      const hoursToLimit = averageHourlyUsage > 0 ? remainingDailyLimit / averageHourlyUsage : Infinity;
  ...
  private detectProviderAndModel(metadata: Record<string, unknown>): { provider?: string; model?: string } {
  ...
  private calculateCost(provider: ProviderConfig, model: ModelConfig, inputTokens: number, outputTokens: number): number {
    return (inputTokens / 1000) * model.pricing.input + (outputTokens / 1000) * model.pricing.output;
  }

  private calculateUsageInPeriod(records: TokenUsageRecord[], startTime: Date, endTime: Date): number {
    return records
  ...
  private checkLimits(provider: ProviderConfig, agentId: string): void {
    ...
    const dailyUsage = this.calculateUsageInPeriod(
      Array.from(this.usageRecords.values())
        .filter(record => record.providerId === provider.id && record.agentId === agentId),
      dayStart,
      now
    );
  ...
  private startLimitMonitoring(): void {
    setInterval(() => {
      ...
      const criticalPredictions = predictions.filter(p => p.recommendation === 'stop' || p.recommendation === 'caution');
  ...
  private async persistData(): Promise<void> {
    try {
      ...
      logger.error('MultiProviderTokenAccounting', 'Failed to persist data', error instanceof Error ? error : new Error(String(error)));
    }
  }
