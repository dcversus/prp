/**
 * ♫ Enhanced Scanner Core Unit Tests
 */
...
describe('EnhancedScannerCore', () => {
  ...
  describe('Scanning Operations', () => {
    ...
    it('should start scanning operations', async () => {
      const mocks = mockScannerMethods(scanner);

      await scanner.start();

      expect(scanner['isScanning']).toBe(true);
      expect(mocks.mockPerformComprehensiveScan).toHaveBeenCalled();
    });
    ...
    it('should detect PRP files and trigger signal detection', async () => {
      const mocks = mockScannerMethods(scanner);
      const detectSignalsSpy = mocks.mockDetectSignalsInPRP;
  ...
  describe('Error Handling', () => {
    ...
    it('should update error count on scan failures', async () => {
      // Mock scanner methods with error
      const mocks = mockScannerMethods(scanner);
      mocks.mockPerformComprehensiveScan.mockRejectedValue(new Error('Scan error'));
  ...
  describe('Utility Methods', () => {
    ...
    it('should have method mocking available', () => {
      const mocks = mockScannerMethods(scanner);
      expect(mocks.mockEmitScannerEvent).toBeDefined();
      expect(typeof mocks.mockEmitScannerEvent).toBe('function');
    });
