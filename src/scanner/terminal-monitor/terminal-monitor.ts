...
export class TerminalMonitor {
  ...
  private activityBuffers: Map<string, TerminalActivity[]> = new Map();
  private lastActivities: Map<string, Date> = new Map();
  private monitoringProcesses: Map<string, ChildProcess> = new Map();
  private resourceTrackers: Map<string, Timeout> = new Map();
  ...
  async startMonitoring(sessionId: string, paneId: string, pid?: number): Promise<void> {
    ...
    try {
      ...
      this.logger.info('startMonitoring', `Terminal monitoring started for session ${sessionId}`, { sessionId, paneId, pid });

    } catch (error) {
      this.logger.error('startMonitoring', 'Failed to start terminal monitoring', error instanceof Error ? error : new Error(String(error)), {
        sessionId,
        paneId
      });
  ...
  async stopMonitoring(sessionId: string): Promise<void> {
    try {
      ...
      this.logger.info('stopMonitoring', `Terminal monitoring stopped for session ${sessionId}`, { sessionId });

    } catch (error) {
      this.logger.error('stopMonitoring', 'Failed to stop terminal monitoring', error instanceof Error ? error : new Error(String(error)), { sessionId });
    }
  }
   ...
   */
  recordActivity(sessionId: string, activity: Omit<TerminalActivity, 'sessionId' | 'timestamp'>): void {
   ...
   */
  getRecentActivities(sessionId: string, limit: number = 50): TerminalActivity[] {
    const activities = this.activityBuffers.get(sessionId);
    if (!activities) {
      return [];
    }
  ...
  private async startTmuxMonitoring(sessionId: string, paneId: string): Promise<void> {
    ...
    monitorProcess.stderr.on('data', (data) => {
      this.logger.error('startTmuxMonitoring', 'Terminal monitor error', new Error(data.toString()), {
        sessionId
      });
    });
  ...
  private processActivityLine(sessionId: string, line: string): void {
    ...
    const activity: TerminalActivity = {
      ...
      content: content && content.length > 200 ? content.substring(0, 200) + '...' : (content ?? '')
    };
  ...
  private startResourceMonitoring(sessionId: string, pid: number): void {
    const interval = setInterval(async () => {
      try {
        ...
        this.logger.debug('Resource monitoring error', error instanceof Error ? error.message : String(error));
      }
  ...
  private calculateIdlePeriods(activities: TerminalActivity[]): Array<{ start: Date; end: Date; duration: number }> {
  ...
  private async generateResourceSignal(sessionId: string, resourceType: string, value: number): Promise<void> {
    if (!this.config.signalGeneration.enabled) {
      return;
    }
