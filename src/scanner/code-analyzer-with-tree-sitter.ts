...
const scannerLogger = createLayerLogger('scanner');
...
export class CodeAnalyzerWithTreeSitter extends EventEmitter {
  ...
  private readonly parsers = new Map<string, TreeSitterParser>();
  private readonly languageConfigs = new Map<string, LanguageConfig>();
  private readonly analysisCache = new Map<string, { result: CodeAnalysisResult; timestamp: Date }>();
  ...
  async analyzeFile(filePath: string): Promise<CodeAnalysisResult> {
    ...
    try {
      ...
      scannerLogger.debug('CodeAnalyzerWithTreeSitter', 'File analysis completed', {
        filePath,
        language,
  ...
  async analyzeDiff(
    ...
    try {
      ...
      scannerLogger.debug('CodeAnalyzerWithTreeSitter', 'Diff analysis completed', {
        filePath,
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          scannerLogger.warn('CodeAnalyzerWithTreeSitter', 'Failed to analyze file in code map', {
            file,
      ...
      scannerLogger.info('CodeAnalyzerWithTreeSitter', 'Code map generated', {
        directoryPath,
  ...
  clearCache(): void {
    ...
    scannerLogger.debug('CodeAnalyzerWithTreeSitter', 'Analysis cache cleared');
  }
  // Private methods
  private initializeLanguageConfigs(): void {
    ...
    this.languageConfigs.set('java', {
      ...
      classPatterns: [/(?:public\s+)?class\s+(\w+)/g, /(?:public\s+)?interface\s+(\w+)/g],
    ...
    scannerLogger.debug('CodeAnalyzerWithTreeSitter', 'Language configs initialized', {
      languages: Array.from(this.languageConfigs.keys()),
    });
  }
  private detectLanguage(filePath: string): string | null {
    const ext = filePath.split('.').pop()?.toLowerCase();
    for (const [language, config] of Array.from(this.languageConfigs.entries())) {
  ...
  private async getParser(language: string): Promise<TreeSitterParser | null> {
    ...
    try {
      ...
      scannerLogger.warn('CodeAnalyzerWithTreeSitter', 'Failed to load parser', {
        language,
        error: error instanceof Error ? error.message : String(error),
      });
      return null;
  ...
  private async analyzeTree(
    ...
    result.metrics.averageFunctionSize =
      result.metrics.totalFunctions > 0
        ? result.structure.functions.reduce((sum, fn) => sum + fn.bodySize, 0) /
  ...
  ): CodeAnalysisResult['dependencies'] {
    const dependencies: CodeAnalysisResult['dependencies'] = [];
    for (const importInfo of this.extractImports(content, _config)) {
      dependencies.push({
  ...
  private getLinePosition(content: string, index: number): Position {
    ...
    const column =
      beforeIndex.lastIndexOf('\n') >= 0 ? index - beforeIndex.lastIndexOf('\n') - 1 : index;
  ...
  private generateDiffSummary(
    ...
    if (linesAdded > 0 || linesRemoved > 0) {
      parts.push(
        `${linesAdded > 0 ? '+' : ''}${linesAdded} ${linesRemoved > 0 ? '-' : ''}${Math.abs(linesRemoved)} lines`,
      );
    }
