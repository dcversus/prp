...
interface TemplateFileInfo {
  ...
  description?: string;
  condition?: string;
  isRootRelative?: boolean; // For paths like "prp/README.md"
}

export class InitGenerationService {
  private logger = new Logger('InitGenerationService');
  private readonly prpRootPath: string;

  constructor() {
    // Use PathResolver to get the correct package root
    this.prpRootPath = PathResolver.getPackagePath();
  }
  ...
  async generateGovernanceFiles(request: GenerationRequest): Promise<GeneratedContent> {
    const { projectName, projectDescription, author, projectPath, template, prompt } = request;

    this.logger.info(`🔧 Generating governance files for project: ${projectName}`);
  ...
  private async generateAgentsMd(
    ...
    try {
      ...
      const systemPartMatch = templateContent.split('> SYSTEM PART! NEVER EDIT THIS PART! USER SECTION BELOW!');
      const systemPart = systemPartMatch[0] + '\n\n---\n> SYSTEM PART! NEVER EDIT THIS PART! USER SECTION BELOW!\n\n---\n\n';

      // Generate project-specific user section
      const userSection = this.generateUserSection(projectName, projectDescription, author, template);
      ...
      throw new Error(`Failed to generate AGENTS.md: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
`;
  ...
  private async getNextPrpNumber(): Promise<string> {
    try {
      const prpsDir = path.join(this.prpRootPath, 'PRPs');
      await fs.mkdir(prpsDir, { recursive: true });

      const files = await fs.readdir(prpsDir);
      const prpFiles = files.filter(file =>
        file.startsWith('PRP-') && file.endsWith('.md')
      );
      ...
      this.logger.warn('Failed to scan existing PRPs, starting with PRP-001:', error);
      return 'PRP-001';
    }
  }
  ...
  private extractTitleFromPrompt(prompt: string): string | null {
    ...
    const keyPhrases = [
      'Create a', 'Build a', 'Develop a', 'Set up a', 'Implement a',
      'Design a', 'Create an', 'Build an', 'Develop an', 'Set up an'
    ];
  ...
  /**
   * Check if file exists in PRP root or template
   */
  private async fileExistsInPrpOrTemplate(relativePath: string): Promise<boolean> {
    // Try PRP root first
    const prpPath = path.join(this.prpRootPath, relativePath);
    try {
      await fs.access(prpPath);
      return true;
    } catch {
      // Try templates directory
      const templatePath = path.join(this.prpRootPath, 'templates', relativePath);
      try {
  ...
  /**
   * Read file from PRP root or templates with fallback
   */
  private async readFromPrpOrTemplate(relativePath: string): Promise<string | null> {
    // Try PRP root first
    const prpPath = path.join(this.prpRootPath, relativePath);
    try {
      return await fs.readFile(prpPath, 'utf8');
    } catch {
      // Try templates directory
      const templatePath = path.join(this.prpRootPath, 'templates', relativePath);
      try {
        return await fs.readFile(templatePath, 'template', 'utf8');
      } catch {
        return null;
      }
    }
  ...
  /**
   * Process template file variables
   */
  private processTemplateVariables(
    ...
    const variables = {
      ...
      'PROJECT_DESCRIPTION': projectDescription || 'A PRP-managed project',
      'AUTHOR': author || 'PRP Developer',
