...
export class ScaffoldingService {
  ...
  constructor() {
    ...
    logger.info('ScaffoldingService', `ScaffoldingService initialized with templates path: ${this.templatesPath}`);
  }

  /**
   * Resolve path from @dcversus/prp root
   */
  private resolvePrpRootPath(relativePath: string): string {
    // Try different strategies to find PRP root

    // 1. Check if we're in the @dcversus/prp project root (has package.json and templates folder)
    //    This covers local development
    const currentDirCheck = path.join(process.cwd(), relativePath);
    if (existsSync(currentDirCheck)) {
      return path.resolve(currentDirCheck);
    }

    // 2. Check if we're in src directory of @dcversus/prp (go up 2 levels to root)
    //    This covers npm run dev scenario
    const srcDirCheck = path.resolve(process.cwd(), '../../', relativePath);
    if (existsSync(srcDirCheck)) {
      return path.resolve(srcDirCheck);
    }

    // 3. Check if we're in dist directory (go up 2 levels to root)
    //    This covers direct execution of dist/cli.js
    const distDirCheck = path.resolve(process.cwd(), '../../', relativePath);
    if (existsSync(distDirCheck)) {
      return path.resolve(distDirCheck);
    }

    // 4. Walk up directory tree looking for PRP root (has templates folder and AGENTS.md)
    let currentDir = path.resolve(process.cwd());
    let foundPrpRoot = null;

    // Walk up directory tree looking for PRP root (has templates folder and AGENTS.md)
    while (currentDir !== path.dirname(currentDir)) {
      const templatesCheck = path.join(currentDir, 'templates');
      const agentsCheck = path.join(currentDir, 'AGENTS.md');

      if (existsSync(templatesCheck) && existsSync(agentsCheck)) {
        foundPrpRoot = currentDir;
        break;
      }
    ...
    if (foundPrpRoot) {
      const resolvedPath = path.join(foundPrpRoot, relativePath);
    ...
    this.logger.warn(`Could not find PRP root, using fallback path: ${fallbackPath}`);
    return fallbackPath;
  }
    ...
    try {
      ...
      for (const dir of templateDirs) {
        ...
        try {
          ...
          this.logger.warn(`Failed to load template config for ${dir}:`, error);
  ...
  async scaffold(options: ScaffoldOptions): Promise<void> {
    ...
    logger.info('ScaffoldingService', `Successfully scaffolded ${projectName} from ${template} template`);
  }
  ...
  private async copyOptionalFiles(
    ...
    for (const file of selectedFiles) {
      ...
      try {
        ...
        this.logger.warn(`Failed to copy optional file ${file}:`, error);
      }
    }
  }
  ...
  private async copyGovernanceFiles(
    ...
    try {
      ...
      if (generatedContent.agentsMd) {
        ...
        logger.info('ScaffoldingService', 'Generated AGENTS.md with project-specific user section');
      }

      // Write README.md
      if (generatedContent.readmeMd) {
        await fs.writeFile(path.join(targetPath, 'README.md'), generatedContent.readmeMd);
        logger.info('ScaffoldingService', 'Generated README.md for project');
      }
      ...
      for (const file of additionalFiles) {
        ...
        try {
          ...
          logger.info('ScaffoldingService', `Copied governance file: ${file.source}`);
        } catch (error) {
          logger.warn('ScaffoldingService', `Failed to copy governance file ${file.source}:`, error);
        }
      }

    } catch (error) {
      logger.error('ScaffoldingService', 'Failed to generate governance files:', error);
    ...
    try {
      ...
      if (options.prompt) {
        ...
        if (generatedContent.initialPrp) {
          ...
          logger.info('ScaffoldingService', 'Generated initial PRP from user prompt');
          return;
        }
      }
...
`;
      ...
      logger.info('ScaffoldingService', 'Generated default initial PRP');

    } catch (error) {
      logger.error('ScaffoldingService', 'Failed to generate initial PRP:', error);
