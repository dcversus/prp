...
import { LazyImport } from './lazy-loader';
...
export { LazyImport as LazyLoader } from './lazy-loader';
...
export const createLazyLoader = <T>(importFn: () => Promise<T>): LazyImport<T> => {
  return new LazyImport(importFn);
};
// Performance decorator for methods
export const measurePerformanceDecorator = (_name: string) =>
  function (
    _target: unknown,
    ...
    descriptor.value = async function (...args: unknown[]) {
      ...
      try {
        ...
        // logger.debug(`Performance: ${_name} took ${_duration}ms`);
        return result;
      } catch (error) {
        // const _duration = Date.now() - startTime;
        // logger.debug(`Performance: ${_name} failed after ${_duration}ms`, error);
        throw error;
      }
    };
    return descriptor;
...
export class PerformanceManager {
  ...
  private readonly performanceCache = performanceCache;
  ...
  getReport(): PerformanceReport {
    ...
    return {
      ...
      thresholds: {
        ...
        violations: thresholdStatus.violations as unknown as Array<{
          metric: string;
          actual: number;
          threshold: number;
        }>,
      ...
      recommendations: this.generateRecommendations(
        monitorMetrics,
        signalMetrics as unknown as SignalMetrics,
        thresholdStatus as unknown as ThresholdStatus,
      ),
    };
  }
  private generateRecommendations(
    ...
    if (!thresholdStatus.passed) {
      recommendations.push(
        'Performance thresholds exceeded, review system resources and optimization strategies',
      );
    }
    const cacheStats = Object.values(this.performanceCache.getAllStats());
    const cacheHitRate =
      cacheStats.length > 0
        ? cacheStats.reduce((acc: number, stat) => acc + (stat.hitRate ?? 0), 0) / cacheStats.length
        : 0;
    if (cacheHitRate < 0.7) {
  ...
  private startMemoryMonitoring(): void {
    setInterval(() => {
      ...
      if (memUsage.heapUsed > 40 * 1024 * 1024) {
        // 40MB
      ...
      if (memUsage.heapUsed > 60 * 1024 * 1024 && global.gc) {
        // 60MB
        global.gc();
      }
  ...
  async cached<T>(key: string, fn: () => Promise<T>): Promise<T> {
    const cached = this.performanceCache.getConfigCache().get(key) as T | undefined;
    if (cached !== undefined) {
...
export const memoize = <T extends (...args: unknown[]) => unknown>(fn: T, cacheSize = 100): T => {
