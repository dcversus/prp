...
export class BenchmarkRunner {
  ...
  async runAllTests(): Promise<BenchmarkSuite> {
    ...
    for (const test of this.tests) {
      logger.debug('shared', 'test-runner', `Running performance test: ${test.constructor.name}`);
  ...
  private generateRecommendations(
    ...
    if (avgDuration > 2000) {
      recommendations.push(
        'Average startup time exceeds 2 seconds. Consider more aggressive lazy loading.',
      );
    }
    if (avgMemory > 50 * 1024 * 1024) {
      recommendations.push(
        'Average memory usage exceeds 50MB. Review memory management and caching strategies.',
      );
    }
    const failedTests = results.filter((r) => !r.success);
    if (failedTests.length > 0) {
      recommendations.push(
        `${failedTests.length} tests failed. Review error handling and edge cases.`,
      );
    }
    const slowTests = results.filter((r) => r.duration > avgDuration * 1.5);
    if (slowTests.length > 0) {
      recommendations.push(
        `${slowTests.length} tests are significantly slower than average. Investigate bottlenecks.`,
      );
    }
...
export const runPerformanceTests = async (): Promise<BenchmarkSuite> => {
  logger.debug('shared', 'test-suite', '🚀 Starting PRP CLI Performance Tests\n');
  const testSuite = new CLIPerformanceTests();
  const results = await testSuite.runAllTests();
  logger.debug('shared', 'test-suite', '\n📊 Performance Test Results:');
  logger.debug('shared', 'test-suite', '============================');
  logger.debug('shared', 'test-suite', `Total Tests: ${results.summary.totalTests}`);
  logger.debug('shared', 'test-suite', `Passed: ${results.summary.passedTests}`);
  logger.debug('shared', 'test-suite', `Failed: ${results.summary.failedTests}`);
  logger.debug('shared', 'test-suite', `Average Duration: ${Math.round(results.summary.averageDuration)}ms`);
  logger.debug('shared', 'test-suite', `Average Memory: ${Math.round(results.summary.averageMemoryUsage / 1024 / 1024)}MB`);
  logger.debug('shared', 'test-suite', `Fastest Test: ${results.summary.fastestTest}`);
  logger.debug('shared', 'test-suite', `Slowest Test: ${results.summary.slowestTest}`);
  if (results.summary.recommendations.length > 0) {
    logger.debug('shared', 'test-suite', '\n💡 Recommendations:');
    results.summary.recommendations.forEach((rec) => logger.debug('shared', 'test-suite', `  - ${rec}`));
  }
  return results;
};
// Individual test runners for specific scenarios
export const testCLIStartup = async (): Promise<PerformanceTestResult> => {
