...
const logger = createLayerLogger('lazy-loader');
...
export class ModulePreloader {
  private static preloadQueue: Set<() => Promise<unknown>> = new Set();
...
export class ConditionalLoader {
  private static loadedModules: Map<string, unknown> = new Map();

  static async loadCommand(commandName: string): Promise<unknown> {
    if (this.loadedModules.has(commandName)) {
      logger.debug('ConditionalLoader', `Command ${commandName} already loaded, returning cached module`);
