...
export class InitGenerationService {
  ...
  async generateGovernanceFiles(request: GenerationRequest): Promise<GeneratedContent> {
    ...
    this.logger.info('shared', 'InitGenerationService', `🔧 Generating governance files for project: ${projectName}`);
  ...
  private async generateAgentsMd(
    ...
    try {
      ...
      this.logger.error('shared', 'InitGenerationService', 'Failed to generate AGENTS.md:', error instanceof Error ? error : new Error(String(error)));
...
`;
  ...
  private async getNextPrpNumber(): Promise<string> {
    try {
      ...
      this.logger.warn('shared', 'InitGenerationService', 'Failed to scan existing PRPs, starting with PRP-001:', { error: error instanceof Error ? error.message : String(error) });
      return 'PRP-001';
    }
  }
