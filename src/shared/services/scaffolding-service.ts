...
import type { GenerationRequest } from './init-generation-service';
...
export class ScaffoldingService {
  ...
  private readonly initGenerationService: InitGenerationService;
  private readonly mcpConfigurator: MCPConfigurator;
  ...
  constructor() {
    ...
    this.logger.info(
      'shared',
      'ScaffoldingService',
      `ScaffoldingService initialized with templates path: ${this.templatesPath}`,
    );
  }
  /**
      ...
      for (const dir of templateDirs) {
        ...
        try {
          ...
          this.logger.warn(
            'shared',
            'ScaffoldingService',
            `Failed to load template config for ${dir}:`,
  ...
  async scaffold(options: ScaffoldOptions): Promise<void> {
    ...
    if (options.mcpConfig?.enabled) {
      ...
      await this.mcpConfigurator.setupEnvironmentVariables(options.mcpConfig.servers, targetPath);
    }
    ...
    this.logger.info(
      'shared',
      'ScaffoldingService',
      `Successfully scaffolded ${projectName} from ${template} template`,
    );
  }
  /**
  ...
  private async copyOptionalFiles(
    ...
    for (const file of selectedFiles) {
      ...
      try {
        ...
        this.logger.warn('shared', 'ScaffoldingService', `Failed to copy optional file ${file}:`, {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    }
  ...
  private async copyGovernanceFiles(
    ...
    try {
      ...
      const generatedContent =
        await this.initGenerationService.generateGovernanceFiles(generationRequest);
      // Write AGENTS.md
      if (generatedContent.agentsMd) {
        ...
        this.logger.info(
          'shared',
          'ScaffoldingService',
          'Generated AGENTS.md with project-specific user section',
        );
      }
      // Write README.md
      ...
      for (const file of additionalFiles) {
        ...
        try {
          ...
          this.logger.info(
            'shared',
            'ScaffoldingService',
            `Copied governance file: ${file.source}`,
          );
        } catch (error) {
          this.logger.warn(
            'shared',
            'ScaffoldingService',
            `Failed to copy governance file ${file.source}:`,
            { error: error instanceof Error ? error.message : String(error) },
          );
        }
      }
      ...
      this.logger.error(
        'shared',
        'ScaffoldingService',
        'Failed to generate governance files:',
        error instanceof Error ? error : new Error(String(error)),
      );
      throw new Error(
        `Failed to generate governance files: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  /**
      ...
      if (options.prompt) {
        ...
        if (generatedContent.initialPrp) {
          const prpPath = path.join(
            prpsDir,
            `PRP-001-initial-${variables.PROJECT_NAME_KEBAB}.md`,
          );
          await fs.writeFile(prpPath, generatedContent.initialPrp);
          this.logger.info(
            'shared',
            'ScaffoldingService',
            'Generated initial PRP from user prompt',
          );
          return;
        }
      }
      // Fallback to default PRP if no prompt provided
      const prpContent = `# PRP-001: Initial ${projectName} Setup
...
`;
      const prpPath = path.join(prpsDir, `PRP-001-initial-${variables.PROJECT_NAME_KEBAB}.md`);
      ...
      this.logger.error(
        'shared',
        'ScaffoldingService',
        'Failed to generate initial PRP:',
        error instanceof Error ? error : new Error(String(error)),
      );
      throw new Error(
        `Failed to generate initial PRP: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  /**
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          'shared',
          'ScaffoldingService',
          `Source file ${sourceFile} does not exist, skipping symlink creation`,
        );
        return;
      }
      ...
      this.logger.info(
        'shared',
        'ScaffoldingService',
        `✅ Created symbolic link: ${targetFile} -> ${sourceFile}`,
      );
    } catch (error) {
      this.logger.warn(
        'shared',
        'ScaffoldingService',
        `Failed to create symbolic link ${targetFile} -> ${sourceFile}:`,
        { error: error instanceof Error ? error.message : String(error) },
      );
    }
  }
