...
export class SignalTracker extends EventEmitter {
  private activeSignals: Map<string, SignalLifecycleEvent[]> = new Map();
   ...
   */
  getAnalytics(days: number = 7): SignalAnalytics {
   ...
   */
  getHistory(filter?: SignalFilter, limit: number = 100): SignalLifecycleEvent[] {
  ...
  getActiveSignals(): Array<{ signalId: string; events: SignalLifecycleEvent[] }> {
    ...
    for (const [signalId, events] of this.activeSignals) {
      result.push({
        signalId,
  ...
  clearHistory(olderThanDays: number = 30): void {
    ...
    if (removedCount > 0) {
      this.logger.info('orchestrator', 'SignalTracker', `Cleared ${removedCount} old signal events`, {
        olderThanDays,
  ...
  private _calculateHourlyVolume(events: SignalLifecycleEvent[]): Array<{ hour: number; count: number }> {
  ...
  private _calculateDailyVolume(events: SignalLifecycleEvent[]): Array<{ date: string; count: number }> {
  ...
  private _calculateProcessingTimes(events: SignalLifecycleEvent[]): Array<{ type: string; min: number; max: number; avg: number }> {
  ...
  private _calculateErrorPatterns(events: SignalLifecycleEvent[]): Array<{ type: string; count: number; lastOccurrence: Date }> {
