...
export interface SignalConsumer {
  ...
  process: (signal: Signal) => Promise<boolean>;
}
...
export class SignalPriorityQueue {
  private queues: Map<number, QueuedSignal[]> = new Map();
  private consumers: SignalConsumer[] = [];
  ...
  enqueue(signal: Signal): boolean {
    ...
    if (totalQueued >= this.maxQueueSize) {
      this.logger.warn('orchestrator', 'SignalPriorityQueue', `Queue at capacity (${this.maxQueueSize}), rejecting signal`, {
        signalId: signal.id,
        type: signal.type
      });
      return false;
    ...
    if (!queue) {
      this.logger.error('orchestrator', 'SignalPriorityQueue', `Invalid priority: ${priority}`, new Error(`Invalid priority: ${priority}`));
      return false;
    }
    ...
    this.logger.debug('orchestrator', 'SignalPriorityQueue', `Signal queued with priority ${priority}`, {
      signalId: signal.id,
      type: signal.type,
      priority,
  ...
  complete(signalId: string, success: boolean): void {
    if (!this.processing.has(signalId)) {
      this.logger.warn('orchestrator', 'SignalPriorityQueue', `Signal ${signalId} not in processing set`);
      return;
    }
  ...
  registerConsumer(consumer: SignalConsumer): void {
    ...
    this.logger.info('orchestrator', 'SignalPriorityQueue', `Consumer registered: ${consumer.name}`, {
  ...
  unregisterConsumer(consumerId: string): void {
    ...
    if (index !== -1) {
      ...
      this.logger.info('orchestrator', 'SignalPriorityQueue', `Consumer unregistered: ${consumer.name}`, {
        consumerId
      });
    }
  }
  ...
  getStats(): QueueStats {
    ...
    for (const [priority, queue] of this.queues) {
      ...
      for (const signal of queue) {
        const category = signal.metadata?.category || 'unknown';
   ...
   */
  getRecentCompleted(limit: number = 50): QueuedSignal[] {
  ...
  clearQueue(): void {
    ...
    this.logger.warn('orchestrator', 'SignalPriorityQueue', `Queue cleared, ${totalCleared} signals removed`);
  }
  ...
  private _updateWaitTimes(): void {
    ...
    for (let priority = 10; priority >= 1; priority--) {
      ...
      for (let i = 0; i < queue.length; i++) {
        queue[i].estimatedWaitTime = cumulativeWait;
  ...
  async processSignals(): Promise<void> {
    ...
    for (const consumer of this.consumers) {
      ...
      try {
        this.logger.debug('orchestrator', 'SignalPriorityQueue', `Processing signal with ${consumer.name}`, {
          signalId: signal.id,
          type: signal.type
        });
        ...
        this.logger.error('orchestrator', 'SignalPriorityQueue', `Error processing signal with ${consumer.name}`, error instanceof Error ? error : new Error(String(error)), {
          signalId: signal.id,
          type: signal.type,
          consumerId: consumer.id
        });
  ...
  startProcessing(intervalMs: number = 1000): () => void {
    this.logger.info('orchestrator', 'SignalPriorityQueue', `Starting automatic signal processing (interval: ${intervalMs}ms)`);
    ...
    return () => {
      ...
      this.logger.info('orchestrator', 'SignalPriorityQueue', 'Stopped automatic signal processing');
    };
  }
}
...
export const DefaultSignalConsumers = {
  orchestrator: {
    ...
    canProcess: (signal: Signal) => {
      ...
      const orchestratorHandledCategories = [
        'system_info', 'agent_info', 'development', 'testing',
        'coordination', 'deployment', 'design'
      ];
      return orchestratorHandledCategories.includes(signal.metadata?.category as string);
    },
    process: async (signal: Signal): Promise<boolean> => {
  ...
  admin: {
    ...
    canProcess: (signal: Signal) => {
      ...
      return signal.priority >= 8 || signal.metadata?.category === 'incident';
    },
    process: async (signal: Signal): Promise<boolean> => {
