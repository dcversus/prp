...
import { createLayerLogger, FileUtils, TimeUtils, HashUtils } from './index';
...
export class StorageManager {
  ...
  private async loadState(): Promise<void> {
    try {
      ...
      logger.info(
        'StorageManager',
        `State loaded successfully: v${loadedState.version}, ${Object.keys(loadedState.worktrees).length} worktrees, ${Object.keys(loadedState.agents).length} agents, ${Object.keys(loadedState.prps).length} PRPs`,
      );
    } catch (error) {
      logger.error(
        'StorageManager',
  ...
  private async loadKeychain(): Promise<void> {
    try {
      ...
      logger.info(
        'StorageManager',
        `Keychain loaded successfully: ${keychainData.entries.length} entries, encrypted: ${keychainData.encrypted}`,
      );
    } catch (error) {
      logger.error(
        'StorageManager',
        'Failed to load keychain',
        error instanceof Error ? error : new Error(String(error)),
      );
    }
  }
  ...
  async updateTokenUsage(
    ...
    const {accounting} = this.state.tokens;
    // Update totals
  ...
  getNotesByPattern(pattern: string): NoteState[] {
    return Object.values(this.state.notes).filter(
      (note) => note.pattern === pattern || note.name.includes(pattern),
    );
  }
  ...
  async getStorageStats(): Promise<StorageStats> {
    ...
    try {
      ...
      logger.warn('StorageManager', 'Failed to calculate storage stats', {
  ...
  async createBackup(description?: string): Promise<string> {
    ...
    logger.info(
      'StorageManager',
      `Backup created: ${backupId} (${backup.metadata.size} bytes) -> ${backupPath}`,
    );
    return backupId;
  }
  async restoreBackup(backupId: string): Promise<void> {
    ...
    try {
      ...
      logger.info(
        'StorageManager',
        `Backup restored: ${backupId} (v${backup.metadata.version} from ${backup.metadata.createdAt})`,
      );
    } catch (error) {
      logger.error(
        'StorageManager',
  ...
  private async performCleanup(): Promise<void> {
    ...
    try {
      ...
      logger.info(
        'StorageManager',
        `Cleanup completed: ${signalsCleaned} signals, ${timeSeriesCleaned} time series, ${errorsCleaned} errors (cutoff: ${cutoffDate.toISOString()})`,
      );
    } catch (error) {
      logger.error(
        'StorageManager',
