...
export class PRPError extends Error {
  ...
  constructor(message: string, code = 'UNKNOWN_ERROR', exitCode = 1, recoverable = false) {
    super(message);
    this.name = 'PRPError';
    this.code = code;
...
export class BuildError extends PRPError {
  constructor(message: string, stage?: string) {
    super(`Build error: ${message}${stage ? ` (stage: ${stage})` : ''}`, 'BUILD_ERROR', 4, false);
    this.name = 'BuildError';
  }
}
export class TestError extends PRPError {
  constructor(message: string, test?: string) {
    super(`Test error: ${message}${test ? ` (test: ${test})` : ''}`, 'TEST_ERROR', 5, false);
    this.name = 'TestError';
  }
}
...
export class NetworkError extends PRPError {
  constructor(message: string, url?: string) {
    super(`Network error: ${message}${url ? ` (url: ${url})` : ''}`, 'NETWORK_ERROR', 9, true);
...
export class ErrorHandler {
   ...
   */
  async handle(error: Error | unknown): Promise<CommandResult> {
  ...
  private logError(error: Error | unknown): void {
    if (error instanceof PRPError) {
      ...
      logger.error('error-handler', `Unexpected error: ${error.message}`);
    } else {
      logger.error('error-handler', `Unknown error: ${String(error)}`);
    }
    // Log stack trace in debug mode
    if (this.debug && error instanceof Error && error.stack) {
      logger.debug('error-handler', error.stack);
    }
  }
  /**
   * Get exit code for error
   */
  private getExitCode(error: Error | unknown): number {
    if (error instanceof PRPError) {
  ...
  private getErrorCode(error: Error | unknown): string {
    ...
    return (error instanceof Error ? error.name : 'UNKNOWN_ERROR') || 'UNKNOWN_ERROR';
  }
  /**
  ...
  private isRecoverable(error: Error | unknown): boolean {
    ...
    return error instanceof Error && recoverableErrors.includes(error.name);
  }
  /**
   * Show suggestions for recoverable errors
   */
  private showSuggestions(error: Error | unknown): void {
    logger.info('error-handler', '\nSuggestions:');
    if (error instanceof ConfigurationError) {
      logger.info('error-handler', '• Run `prp config validate` to check your configuration');
      logger.info('error-handler', '• Use `prp config edit` to open configuration file');
      logger.info('error-handler', '• Check the documentation at /docs/config/README.md');
    }
    if (error instanceof ValidationError) {
      logger.info('error-handler', '• Check the required fields and their formats');
      logger.info('error-handler', '• Run `prp config validate` for detailed validation');
      logger.info('error-handler', '• Refer to the configuration schema');
    }
    if (error instanceof DependencyError) {
      logger.info('error-handler', '• Run `prp deps install` to install dependencies');
      logger.info('error-handler', '• Try `prp deps update` to update dependencies');
      logger.info('error-handler', '• Check your package.json configuration');
    }
    if (error instanceof NetworkError) {
      logger.info('error-handler', '• Check your internet connection');
      logger.info('error-handler', '• Verify firewall and proxy settings');
      logger.info('error-handler', '• Try again in a few moments');
    }
    if (error instanceof AuthenticationError) {
      logger.info('error-handler', '• Run `prp auth login` to authenticate');
      logger.info('error-handler', '• Check your credentials');
      logger.info('error-handler', '• Verify service access permissions');
    }
    if (error instanceof PermissionError) {
      logger.info('error-handler', '• Check file and directory permissions');
      logger.info('error-handler', '• Try running with elevated privileges if necessary');
      logger.info('error-handler', '• Verify you have write access to the target directory');
    }
    logger.info('error-handler', '• Run with --debug flag for more information');
    logger.info('error-handler', '• Use `prp --help` for command assistance');
  }
  /**
  ...
  async handleMultiple(errors: Error[]): Promise<CommandResult> {
    ...
    errors.slice(1).forEach((error) => {
      if (error instanceof Error) {
        ...
        logger.warn(`Additional error: ${String(error)}`);
      }
    });
