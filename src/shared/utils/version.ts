...
export function getVersion(): string {
  ...
  try {
    ...
    logger.debug('version', 'Could not read version from package.json, using fallback:', error instanceof Error ? error.message : String(error));
    return '0.0.0';
  }
}
...
export function getPackageVersion(): string {
  try {
    ...
    logger.debug('version', 'Could not read package name from package.json, using fallback:', error instanceof Error ? error.message : String(error));
