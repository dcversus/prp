...
export class MetricsCalculator {
  ...
  static calculateMetrics(
    ...
    const tokenUsages = results
      .filter((r): r is OperationResult & { tokenUsage: NonNullable<OperationResult['tokenUsage']> } => r.tokenUsage !== undefined)
      .map((r) => r.tokenUsage);
   ...
   */
  static calculateProcessingRate(totalOperations: number, timeWindowMs: number): number {
    if (timeWindowMs === 0) {
      return 0;
    }
  ...
  static calculatePeakThroughput(results: OperationResult[]): number {
    ...
    const maxOperations = Math.max(...Object.values(minuteWindows).map((window) => window.length));
   ...
   */
  static calculateTrends(historicalMetrics: PerformanceMetrics[]): {
    ...
    return {
      processingRateTrend: this.getTrend(
        recentAvg.processingRate,
        olderAvg.processingRate,
        threshold,
      ),
...
export class PerformanceMonitor {
  ...
  private readonly operationHistory = new Map<string, OperationResult[]>();
  ...
  private updateComponentMetrics(component: string): void {
    ...
    this.metrics.set(
      component,
      MetricsCalculator.calculateMetrics(history, startTime, currentQueueLength),
    );
  }
  /**
