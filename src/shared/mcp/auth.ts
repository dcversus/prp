...
export class MCPAuth {
  ...
  verifyToken(token: string): MCPClaim | null {
    try {
      ...
      logger.warn('shared', 'JWT verification failed:', error instanceof Error ? error : new Error(String(error)));
      return null;
    }
  }
   ...
   */
  getAuthMiddleware(requiredPermissions: string[] = []): (req: Request, res: Response, next: NextFunction) => void {
