...
export class NudgeClient {
  ...
  constructor(options: NudgeClientOptions = {}) {
    ...
    if (!this.config.secret) {
      throw new NudgeError('CONFIG_ERROR', 'NUDGE_SECRET is required but not configured', {
        config: this.config,
      });
    }
  ...
  async sendNudge(request: NudgeRequest): Promise<NudgeResponse> {
    ...
    const attemptSend = async (attempt: number): Promise<NudgeResponse> => {
      try {
        const response: AxiosResponse<NudgeResponse> = await this.client.post('', request);
        return {
          success: true,
          ...
          delivery_type:
            response.data.delivery_type ?? (request.type === 'direct' ? 'direct' : 'llm-enhanced'),
        };
      } catch (error) {
        ...
        if (this.shouldRetry(error, attempt)) {
          ...
          logger.warn(
            `Nudge send failed (attempt ${attempt}/${this.config.retry_attempts}), retrying in ${delay}ms...`,
          );
  ...
  private validateNudgeRequest(request: NudgeRequest): void {
    ...
    if (!request.message || typeof request.message !== 'string') {
      throw new NudgeError('VALIDATION_ERROR', 'Message is required and must be a string', {
        request,
      });
    }
    if (request.type === 'direct' && !request.urgency) {
      throw new NudgeError('VALIDATION_ERROR', 'Direct nudge requires urgency level', { request });
    }
  ...
  private handleError(error: Error | AxiosError | unknown, attempt: number): NudgeError {
    if (axios.isAxiosError(error)) {
      ...
      switch (status) {
        ...
        case 403:
          return new NudgeError('AUTHORIZATION_ERROR', 'Not authorized to send nudges', {
            status,
            data,
            attempt,
          });
        case 429:
          return new NudgeError('RATE_LIMIT_ERROR', 'Rate limit exceeded. Please try again later', {
            status,
            data,
            attempt,
          });
        ...
        case 503:
        case 504:
          return new NudgeError('SERVER_ERROR', 'dcmaidbot server error', {
            status,
            data,
            attempt,
          });
        default:
          return new NudgeError('HTTP_ERROR', `HTTP error ${status}: ${axiosError.message}`, {
            status,
            data,
            attempt,
  ...
  private shouldRetry(error: Error | AxiosError | unknown, attempt: number): boolean {
    ...
    if (error instanceof NudgeError) {
      return !['VALIDATION_ERROR', 'AUTHENTICATION_ERROR', 'AUTHORIZATION_ERROR'].includes(
        error.code,
      );
    }
    return true;
