...
class FileSelfStore implements SelfStore {
  ...
  async get(): Promise<SelfData | null> {
    try {
      ...
      logger.debug('self-store', 'get', 'Failed to read self data', error instanceof Error ? error : new Error(String(error)));
      return null;
    }
  }
  ...
  async set(data: SelfData): Promise<void> {
    try {
      ...
      logger.error('self-store', 'set', 'Failed to store self data', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async clear(): Promise<void> {
    try {
      ...
      logger.debug('self-store', 'clear', 'No self data to clear', error instanceof Error ? error : new Error(String(error)));
    }
  }
