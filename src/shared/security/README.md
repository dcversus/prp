...
if (!result.isValid) {
  // Import logger for error output: import { logger } from '../logger.js';
  // logger.error('Invalid input:', result.error);
  process.exit(1);
}
...
if (credential) {
  ...
  logger.debug('Retrieved:', credential.name);
  logger.debug('Type:', credential.type);
...
credentials.forEach(cred => {
  // Import logger: import { logger } from '../logger.js';
  logger.debug(`${cred.name} (${cred.type}) - Created: ${cred.createdAt}`);
});
```
