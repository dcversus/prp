...
export class CredentialManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info('shared', 'CredentialManager', 'initialized successfully');
    } catch (error) {
      logger.error('shared', 'CredentialManager', 'Failed to initialize CredentialManager', error instanceof Error ? error : new Error(String(error)));
      throw new Error(
        `CredentialManager initialization failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  /**
  ...
  async storeCredential(credential: Omit<Credential, 'id' | 'createdAt'>): Promise<string> {
    ...
    try {
      ...
      if (this.config.enableAccessLogging) {
        logger.info('shared', `Stored credential: ${credential.name} (${credential.type})`);
      }
      return id;
    } catch (error) {
      logger.error('shared', 'Failed to store credential', error);
      throw new Error(
  ...
  async getCredential(id: string): Promise<Credential | null> {
    ...
    try {
      ...
      if (this.config.enableAccessLogging) {
        logger.info('shared', `Retrieved credential: ${credential.name} (${credential.type})`);
      }
      // Check if credential has expired
      if (credential.expiresAt && credential.expiresAt < new Date()) {
        logger.warn('shared', `Retrieved expired credential: ${credential.name}`);
      }
      return credential;
    } catch (error) {
      logger.error('shared', 'Failed to retrieve credential', error);
      throw new Error(
  ...
  async listCredentials(): Promise<Omit<Credential, 'value'>[]> {
    ...
    try {
      ...
      if (this.config.enableAccessLogging) {
        logger.info('shared', `Listed ${credentials.length} credentials`);
      }
      return credentials;
    } catch (error) {
  ...
  async searchCredentials(query: {
    ...
    try {
      ...
      throw new Error(
        `Failed to search credentials: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  /**
  ...
  async rotateMasterKey(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to rotate master key: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  /**
  ...
  async cleanupExpiredCredentials(): Promise<number> {
    ...
    try {
      ...
      throw new Error(
        `Failed to cleanup expired credentials: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  /**
  ...
  private async loadStorage(): Promise<CredentialStorage> {
    try {
      ...
      throw new Error(
        `Failed to load credential storage: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  private async saveStorage(storage: CredentialStorage): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to save credential storage: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  private async updateStorage(
    updateFn: (storage: CredentialStorage) => Promise<void>,
  ): Promise<void> {
    const storage = await this.loadStorage();
  ...
  private async encryptCredentialWithKey(
  ...
  private async decryptCredentialWithKey(
