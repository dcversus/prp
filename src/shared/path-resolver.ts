  ...
  static getPackageRoot(): string {
    ...
    try {
      ...
      logger.info(
        'shared',
        'PathResolver',
        `Package root found via traversal: ${this.packageRoot}`,
      );
      return this.packageRoot;
    } catch (error) {
      ...
      logger.error(
        'shared',
        'PathResolver',
        `Failed to resolve package root. Execution: ${execInfo.type}, Module: ${execInfo.modulePath}, Error: ${error}`,
      );
      throw new Error(
        `Failed to resolve package root directory: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  /**
  ...
  static getPackagePath(relativePath: string): string {
    ...
    if (!existsSync(fullPath)) {
      ...
      logger.warn(
        'shared',
        'PathResolver',
        `Path does not exist: ${fullPath}. Execution: ${execInfo.type}, PackageRoot: ${this.packageRoot}`,
      );
    }
    return fullPath;
  }
