...
import type { ChannelEvent, ScannerEvent, InspectorEvent, OrchestratorEvent, Signal } from './types';
...
export class EventChannelImpl<T = Record<string, unknown>> implements EventChannel<T> {
  ...
  private readonly subscribers = new Set<(event: ChannelEvent<T>) => void>();
    ...
    this.subscribers.forEach((callback) => {
      try {
        ...
        logger.error(
          'shared',
          'EventChannel',
          `Error in event subscriber for channel ${this.name}`,
  ...
  getRecent(count = 100): ChannelEvent<T>[] {
...
export class EventBus implements IEventBus {
  ...
  createChannel<T>(name: string, maxEvents = 1000): EventChannel<T> {
  ...
  publishSignal(signal: Signal): void {
    const event: ChannelEvent<Signal> = {
      id: this.generateEventId(),
      type: 'signal',
      timestamp: new Date(),
    ...
    this.publishToChannel('signals', event);
  }
  ...
  getChannelStats(): Record<string, ChannelStats> {
    ...
    for (const [name, channel] of channelEntries) {
      ...
      stats[name] =
        'getStats' in channel && typeof channel.getStats === 'function'
          ? channel.getStats()
 ...
 */
export function createEventFilter<T extends ChannelEvent>(predicate: (event: T) => boolean) {
