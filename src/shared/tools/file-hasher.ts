...
const logger = createLayerLogger('file-hasher');
...
export class FileHasher {
  ...
  async hashFile(filePath: string, algorithm: 'md5' | 'sha1' | 'sha256' = 'sha256'): Promise<FileHash> {
    ...
    if (cached) {
      try {
        ...
        if (stats.mtime.getTime() === cached.lastModified.getTime()) {
          logger.debug('FileHasher', `Using cached hash for ${filePath}`);
          return cached;
        }
      } catch (error) {
        logger.debug('FileHasher', `Failed to stat cached file ${filePath}`, error as Error);
      }
    }
    ...
    try {
      ...
      logger.debug('FileHasher', `Calculated new hash for ${filePath} (${algorithm})`);

      return fileHash;
    } catch (error) {
      logger.error('FileHasher', `Failed to hash file ${filePath}`, error as Error);
  ...
  async hasFileChanged(filePath: string, algorithm: 'md5' | 'sha1' | 'sha256' = 'sha256'): Promise<boolean> {
    ...
    if (!cached) {
      logger.debug('FileHasher', `No cached hash for ${filePath}, assuming changed`);
      return true; // No previous hash
    }

    try {
      const stats = statSync(filePath);
      const changed = stats.mtime.getTime() !== cached.lastModified.getTime();
      logger.debug('FileHasher', `File ${filePath} changed: ${changed}`);
      return changed;
    } catch (error) {
      logger.debug('FileHasher', `Failed to stat file ${filePath}, assuming changed`, error as Error);
  ...
  getCachedHash(filePath: string, algorithm: 'md5' | 'sha1' | 'sha256' = 'sha256'): FileHash | null {
    ...
    logger.debug('FileHasher', `Retrieved cached hash for ${filePath}: ${cached ? 'found' : 'not found'}`);
    return cached;
  }
  ...
  clearCache(): void {
    this.cache.clear();
    logger.debug('FileHasher', 'Cache cleared');
  }
  ...
  getCacheStats() {
    ...
    logger.debug('FileHasher', `Cache stats: ${JSON.stringify(stats)}`);
    return stats;
  }
}