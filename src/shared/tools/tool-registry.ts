...
import { Tool, ToolResult, ExecutionStats, RateLimit } from './types.js';
...
export class ToolRegistry extends EventEmitter {
  ...
  async executeTool(toolId: string, parameters: ToolParameters): Promise<ToolResult> {
    ...
    try {
      ...
      stats.averageExecutionTime = (stats.averageExecutionTime * (stats.calls - 1) + executionTime) / stats.calls;
      stats.successes++;
      ...
      logger.info('ToolRegistry', `Tool executed: ${toolId} in ${executionTime}ms - Success: ${result.success}`);
  ...
  private validateTool(tool: Tool): void {
    const requiredFields = ['id', 'name', 'description', 'category', 'enabled', 'parameters', 'execute'];
    ...
    if ('type' in paramDefs && paramDefs.type === 'object') {
      ...
      if (Array.isArray(paramDefs)) {
        ...
        for (const [paramName, paramDef] of Object.entries(paramDefs)) {
          ...
          if (paramName in parameters && !this.validateParameterType(parameters[paramName], paramDef)) {
            return `Invalid type for parameter ${paramName}: expected ${paramDef.type}`;
          }
        }
      }
   ...
   */
  private validateParameterType(value: unknown, paramDef: any): boolean {
    if (!('type' in paramDef)) {
      return true;
    }
   ...
   * Execute tool with timeout
   */
  private async executeWithTimeout(tool: Tool, parameters: ToolParameters, timeout: number): Promise<ToolResult> {
