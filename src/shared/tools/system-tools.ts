/**
 * ♫ System Tools for @dcversus/prp
 *
 * Generic system administration tools including bash commands, file operations,
 * and process management. Can be used by orchestrator, agents, and other components.
 */
...
const logger = createLayerLogger('system-tools');
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (
          typedParams.filter &&
          !command.toLowerCase().includes(typedParams.filter.toLowerCase())
        ) {
          continue;
        }
        processes.push({
...
export const fileSystemTool: Tool = {
  ...
  execute: async (params: unknown) => {
    ...
    try {
      ...
      logger.error(
        'file_system',
        `File system operation failed: ${typedParams.operation}`,
        error instanceof Error ? error : new Error(String(error)),
      );
      return {
        success: false,
...
export const systemTools = [bashCommandTool, processListTool, fileSystemTool, systemInfoTool];
