...
import { createLayerLogger, configManager } from ".";
...
export class GitHubClient {
  ...
  constructor(config?: Partial<GitHubConfig>) {
    ...
    const token =
      config?.token ??
      (prpConfig.agents.find((a) => a.type.includes('github'))?.configuration?.[
        'token'
      ] as string) ??
  ...
  async analyzePR(prNumberOrUrl: string | number): Promise<PRAnalysis> {
    const prNumber =
      typeof prNumberOrUrl === 'string' ? this.extractPRNumber(prNumberOrUrl) : prNumberOrUrl;
  ...
  async getPR(prNumber: number): Promise<PullRequestInfo> {
    const response = await this.client.get<GitHubPR>(
      `/repos/${  this.config.defaultOwner  }/${  this.config.defaultRepo  }/pulls/${  prNumber}`,
    );
    const {data} = response;
    return {
      id: data.id,
      ...
      requestedReviewers:
        data.requested_reviewers?.map((reviewer: GitHubReviewer) => reviewer.login) ?? [],
  ...
  async getCIStatus(prNumber: number): Promise<CIStatus> {
    ...
    const prResponse = await this.client.get<GitHubPR>(
      `/repos/${  this.config.defaultOwner  }/${  this.config.defaultRepo  }/pulls/${  prNumber}`,
    );
    const headSha = prResponse.data.head.sha ?? prResponse.data.head.ref;
    // Get combined status
    const statusResponse = await this.client.get<GitHubStatusResponse>(
      `/repos/${ 
  ...
  async getComments(prNumber: number): Promise<PRComment[]> {
    const response = await this.client.get<GitHubComment[]>(
      `/repos/${ 
        this.config.defaultOwner 
        }/${ 
        this.config.defaultRepo 
  ...
  async getReviews(prNumber: number): Promise<PRReview[]> {
    const response = await this.client.get<GitHubReview[]>(
      `/repos/${ 
        this.config.defaultOwner 
        }/${ 
        this.config.defaultRepo 
  ...
  async getPRMetadata(prNumber: number): Promise<PRMetadata> {
    const [prResponse, repoResponse] = await Promise.all([
      this.client.get<GitHubPR>(
        `/repos/${  this.config.defaultOwner  }/${  this.config.defaultRepo  }/pulls/${  prNumber}`,
      ),
      this.client.get<GitHubRepo>(
        `/repos/${  this.config.defaultOwner  }/${  this.config.defaultRepo}`,
      ),
    ]);
  ...
  async postComment(prNumber: number, body: string): Promise<PRComment> {
    ...
    const comment = response.data as GitHubComment;
    return {
      id: comment.id,
      author: comment.user.login,
      body: comment.body,
  ...
  async createReview(
    ...
    const data = response.data as GitHubReview;
    return {
      id: data.id,
      author: data.user.login,
      state: data.state.toLowerCase() as PRReview['state'],
      body: data.body ?? '',
  ...
  async searchPRs(
    ...
    const q =
      ...
      state}`;
    const response = await this.client.get<GitHubSearchResponse>('/search/issues', {
      params: { q },
    });
