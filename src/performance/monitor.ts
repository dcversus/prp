...
export class PerformanceMonitor {
  ...
  checkThresholds(): { passed: boolean; violations: string[] } {
    ...
    if (current.startupTime > this.thresholds.startupTime) {
      violations.push(`Startup time ${current.startupTime}ms exceeds threshold ${this.thresholds.startupTime}ms`);
    }

    if (current.memoryUsage.rss > this.thresholds.memoryRSS) {
      violations.push(`RSS memory ${Math.round(current.memoryUsage.rss / 1024 / 1024)}MB exceeds threshold ${Math.round(this.thresholds.memoryRSS / 1024 / 1024)}MB`);
    }

    if (current.memoryUsage.heapUsed > this.thresholds.memoryHeapUsed) {
      violations.push(`Heap memory ${Math.round(current.memoryUsage.heapUsed / 1024 / 1024)}MB exceeds threshold ${Math.round(this.thresholds.memoryHeapUsed / 1024 / 1024)}MB`);
    }
