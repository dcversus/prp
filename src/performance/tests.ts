...
export class PerformanceTest {
  ...
  async runTest(testFn: () => Promise<any>, metrics?: Record<string, any>): Promise<PerformanceTestResult> {
...
export class BenchmarkRunner {
  ...
  addTest(testName: string, _testFn: () => Promise<any>, _metrics?: Record<string, any>): void {
...
export class CLIPerformanceTests extends BenchmarkRunner {
  ...
  private setupTests(): void {
    ...
    this.addTest('CLI Startup Time', async () => {
      const start = Date.now();
      execSync('node dist/cli.js --version', { stdio: 'ignore' });
      const duration = Date.now() - start;

      if (duration > 2000) {
        throw new Error(`CLI startup too slow: ${duration}ms`);
      }
    ...
    this.addTest('CLI Help Command', async () => {
      const start = Date.now();
      execSync('node dist/cli.js --help', { stdio: 'ignore' });
      const duration = Date.now() - start;
    ...
    this.addTest('Memory Usage Under Load', async () => {
      const initialMemory = process.memoryUsage();

      // Simulate load by importing multiple modules
      await Promise.all([
        import('commander'),
        import('chalk'),
        import('fs-extra')
      ...
      if (memoryIncrease > 20 * 1024 * 1024) { // 20MB increase
    ...
    this.addTest('Cache Performance', async () => {
      const cache = performanceCache.getConfigCache();
      ...
      return { writeDuration, readDuration, operations: 1000 };
    });

    // Test file system operations
    this.addTest('File System Operations', async () => {
      const fs = await import('fs-extra');
      const testDir = '/tmp/prp-perf-test';
      ...
      for (let i = 0; i < 100; i++) {
        await fs.writeFile(`${testDir}/file-${i}.txt`, `test content ${i}`);
      }

      // Read files
      for (let i = 0; i < 100; i++) {
        await fs.readFile(`${testDir}/file-${i}.txt`);
      }
      ...
      return { duration, filesCreated: 100 };
    });
  }
}
...
export async function testCLIStartup(): Promise<PerformanceTestResult> {
  ...
  return await test.runTest(async () => {
    const start = Date.now();
    execSync('node dist/cli.js --version', { stdio: 'ignore' });
    const duration = Date.now() - start;
