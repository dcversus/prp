...
export class TabbedTUI extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to start TUI', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  private renderContent(): void {
    ...
    const maxLines = process.stdout.rows - this.config.layout.tabBar.height - this.config.layout.status.height - 2;
    const lines = content.content.slice(-maxLines);

    for (const line of lines) {
      console.log(`${this.config.colors.text}${line}\x1b[0m`);
    }
  }
  ...
  private renderStatusBar(): void {
    ...
    console.log('─'.repeat(process.stdout.columns));
    console.log(`${status.padEnd(process.stdout.columns - timestamp.length - 1)}${timestamp}`);
  }
  ...
  private getTabColor(tab: TabInfo): string {
    ...
    if (tab.badge) {
      switch (tab.badge.type) {
        ...
        default: return this.config.colors.text;
      }
    }
  ...
  private updateTabBadge(tabId: string, content: string[], _metadata?: Record<string, unknown>): void {
    ...
    const errorCount = content.filter(line =>
      line.toLowerCase().includes('error') ||
      line.toLowerCase().includes('failed')
    ).length;

    const warningCount = content.filter(line =>
  ...
  private updateInfoScreen(): void {
    const content = [
      ...
      '',
      '📋 Available Tabs:',
      ...Array.from(this.tabs.values()).map(t =>
      `   • ${t.title} (${t.type})`
      ),
      '',
      `Last updated: ${new Date().toLocaleTimeString()}`
  ...
  private handleActivityDetected(event: TmuxActivityDetectedEvent): void {
    ...
    const agentTab = Array.from(this.tabs.values()).find(t =>
      t.sessionId === event.sessionId && t.type === 'agent'
    );
